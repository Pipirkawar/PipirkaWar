"""In-memory реализация `IForestRunRepository` для unit-тестов use-case-ов.

Имитирует ключевое поведение `SqlAlchemyForestRunRepository`:
...
- `save()` обновляет запись по `id` (используется в `FinishForestRun`,
  Спринт 1.3.C). Отсутствующий id — `IntegrityError`.
...
@dataclass
class FakeForestRunRepository(IForestRunRepository):
    """In-memory реализация для тестов use-case-ов леса."""

    rows: list[ForestRun] = field(default_factory=list)

    async def add(self, run: ForestRun) -> ForestRun:
        if run.id is not None:
            raise IntegrityError(f"ForestRun with pre-set id={run.id} cannot be added; use save()")
        # Имитация partial unique-индекса: один активный поход на игрока.
        if run.status is ForestRunStatus.IN_PROGRESS and any(
            existing.player_id == run.player_id and existing.status is ForestRunStatus.IN_PROGRESS
            for existing in self.rows
        ):
            raise IntegrityError(f"player_id={run.player_id} already has an active forest run")
    ...
    async def save(self, run: ForestRun) -> ForestRun:
        if run.id is None:
            raise IntegrityError("ForestRun.save requires id; use add() for new runs")
        for i, existing in enumerate(self.rows):
            if existing.id == run.id:
                self.rows[i] = run
                return run
        raise IntegrityError(f"ForestRun id={run.id} does not exist")

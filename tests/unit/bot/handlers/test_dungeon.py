"""Юнит-тесты `/dungeon`-handler-а и dungeon-callback-handler-а (Спринт 3.1-E)."""
...
from pipirik_wars.application.dungeon import DungeonRunStarted, StartDungeonRun
...
from pipirik_wars.bot.handlers.dungeon import (
    handle_dungeon,
    handle_dungeon_callback,
)
...
def _stub_start(*, success: bool = True, error: BaseException | None = None) -> MagicMock:
    use_case = MagicMock(spec=StartDungeonRun)
    ...
    if success:
        ...
        use_case.execute = AsyncMock(return_value=DungeonRunStarted(run=run, cooldown_minutes=30))
    return use_case
...
class TestHandleDungeon:
    async def test_private_success_sends_started_message(self) -> None:
        ...
        assert "ru:dungeon-started" in sent
    ...
    async def test_group_chat_uses_group_key(self) -> None:
        ...
        await handle_dungeon(
            cast(Message, msg),
            _identity("group"),
            cast(StartDungeonRun, _stub_start()),
            cast(GetProfile, _stub_profile()),
            _bundle(),
            _RU,
        )
        msg.answer.assert_awaited_once_with("ru:dungeon-group")
    ...
    async def test_no_identity_uses_other_key(self) -> None:
        ...
        msg.answer.assert_awaited_once_with("ru:dungeon-other")
    ...
    async def test_player_not_found(self) -> None:
        ...
        msg.answer.assert_awaited_once_with("ru:dungeon-not-registered")

    async def test_already_in_dungeon(self) -> None:
        msg = _msg("private")
        start = _stub_start(error=AlreadyInDungeonError(player_id=1))
        await handle_dungeon(
            cast(Message, msg),
            _identity("private"),
            cast(StartDungeonRun, start),
        ...
        msg.answer.assert_awaited_once_with("ru:dungeon-already-in")

    async def test_requirement_thickness(self) -> None:
        msg = _msg("private")
        start = _stub_start(
            error=DungeonRequirementError(
        ...
        sent = msg.answer.await_args.args[0]
        assert "dungeon-requirement-thickness" in sent
    ...
    async def test_requirement_length(self) -> None:
        msg = _msg("private")
        start = _stub_start(
            error=DungeonRequirementError(player_id=1, requirement="length", required=20, actual=15)
        )
        await handle_dungeon(
            cast(Message, msg),
            _identity("private"),
        ...
        sent = msg.answer.await_args.args[0]
        assert "dungeon-requirement-length" in sent
    ...
    async def test_profile_missing_uses_started_fallback(self) -> None:
        ...
        sent = msg.answer.await_args.args[0]
        assert "dungeon-started-fallback" in sent
        assert "cooldown_minutes=30" in sent
...
class TestHandleDungeonCallback:
    ...
    async def test_equip_item_action(self) -> None:
        ...
        assert "dungeon-toast-item-equipped-placeholder" in toast
        cb.message.edit_reply_markup.assert_awaited_once()

    async def test_drop_item_action(self) -> None:
        cb = _callback("dungeon:drop_item:11:1")
        await handle_dungeon_callback(
            cast(CallbackQuery, cb),
            _identity("private"),
            _bundle(),
        ...
        cb.answer.assert_awaited_once()
        toast = cb.answer.await_args.args[0]
        assert "dungeon-toast-item-dropped" in toast
    ...
    async def test_invalid_callback_data_strips_keyboard(self) -> None:
        ...
        cb.answer.assert_awaited_once()
        toast = cb.answer.await_args.args[0]
        assert "dungeon-toast-drop-mismatch" in toast

    async def test_mountains_callback_routed_to_dungeon_handler_is_ignored(self) -> None:
        # Защита от промаха фильтра: mountains-callback в data-handler →

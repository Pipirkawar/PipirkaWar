...
- **C.10 — Юнит- и integration-тесты:**
  ...
  - `tests/integration/db/test_caravan_battle_finish.py` (519 строк, 3 теста): e2e через UoW + настоящие SQLAlchemy-репо: создать караван в IN_BATTLE-статусе с участниками → вызвать `FinishCaravanBattle` → проверить итоговые `users.length_cm` (лидер ×4, защитник ×base_reward, +1 см клан-бонус) + `caravans.status=FINISHED` + audit-записи (`CARAVAN_BATTLE_FINISHED` + `CARAVAN_REWARDS_GRANTED` + 6× `LENGTH_GRANT`) + activity-locks сняты. Идемпотентность повторного вызова. LOBBY-статус → `InvalidCaravanStateError`.
...
- **Clan-bonus = +1 см каждому участнику обоих кланов** (ГДД §9.6: «+1 см каждому»). Это применяется на player-level через `IClanMembershipRepository.list_by_clan(clan_id)` × 2 (sender + receiver), не на clan-aggregate-level (clan-aggregate `length_cm` отсутствует — кланы у нас не имеют собственной длины, длина аккумулируется на уровне игроков).

...
`<обновится после A.5 push>` — `feat(4.1-A): A.5 — Persistence (payments table + Alembic 0026_payments_and_audit_source + SqlAlchemyPaymentLedger + roulette_paid_reward в audit whitelist) + chore: drop unused type:ignore (mypy fix) + docs: CONTRIBUTING.md handoff sticky-mode`.

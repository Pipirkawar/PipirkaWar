...
| **1.6.E** | Гейт `AnticheatGuard` для всех «спендалок» длины (`/upgrade`, в будущем `/duel`/караваны/рейды) — если `anticheat_ban_until > now()` → ошибка «вы в режиме проверки». Подключение к существующему `UpgradeThickness`. Юнит-тесты на каждый гейт. | 🟢 PR open | 1.6.5 |

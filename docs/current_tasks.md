...
| **1.5.G** | Каталог 300+ JSON-шаблонов забавных логов леса (`templates/forest_logs_{ru,en}.json`, рандомайзер с весами по `IRandom`). Подключение к `StartForestRun`/`FinishForestRun` — один из шаблонов выбирается в finished-сообщении. Ревизия audit-логов: каждая ±см пишется через `AuditLogger` с `LENGTH_DELTA` + причина и `length_before`/`length_after`/`delta_cm`. | ⚪ бэклог | 1.5.3 (300+ шаблонов забавных логов), 1.5.4 (audit_log ±см с причиной) |
| **1.5.H** | Базовый `docker-compose.yml` (бот + локальная Postgres + applied migrations). README + CONTRIBUTING + инструкция деплоя на VPS (1 GB RAM + Neon free). Smoke-runbook деплоя в `ops/`. Финальный DoD MVP-чек-лист. | ⚪ бэклог | 1.5.5 (docker-compose), 1.5.6 (README/CONTRIBUTING/deploy), 1.5.7 (деплой MVP на VPS + Neon) |

---

## ⚪ Бэклог — Спринт 1.6 (Анти-чит хардкап, pre-Phase-2)

Полная спецификация — в [ГДД §3.3](pipirik_wars_plan.md) и [`development_plan.md` §4 / Спринт 1.6](development_plan.md). Цель: ввести жёсткий хардкап на organic-прирост длины (3000 см/сутки rolling, 14000 см/неделя rolling) с soft-ban-ом на 14 дней при обходе. Донат не лимитируется. Должен быть готов **до** Фазы 2 (PvP, масс-PvP) — там новые источники длины с высокой пропускной способностью.

| PR | Содержимое | Статус | Задачи из `development_plan.md` §4 |
|---|---|---|---|
| **1.6.A** | БД-фундамент: миграция `users.anticheat_ban_until TIMESTAMPTZ NULL` + `audit_log.clamped_from INT NULL` + `audit_log.source TEXT NOT NULL` (с CHECK на whitelist source-ов из ГДД §3.3.4 и backfill `'unknown'` для старых строк). ORM/repo обновления. Domain `Player.anticheat_ban_until` + `with_anticheat_ban(...)`. Юнит-тесты на миграции и persistence. | ⚪ бэклог | 1.6.1 |
| **1.6.B** | Конфиг: `balance.yaml` секция `anticheat` (`daily_cap_cm: 3000`, `weekly_cap_cm: 14000`, `soft_ban_duration_days: 14`, whitelist `organic_sources` + `donate_sources`). Pydantic-схема + `IBalanceConfig.anticheat` getter + hot-reload через `/balance_reload`. Юнит-тесты. | ⚪ бэклог | 1.6.2 |
| **1.6.C** | Доменная сущность `AnticheatWindow` (rolling-агрегация organic-сумм за окно), порт `IAnticheatRepository.sum_organic_in_window(player_id, since)` + реализация `SqlAlchemyAnticheatRepository` (один SELECT по `audit_log` с фильтром по `source IN (...organic_sources...)` и `delta > 0`). Юнит-тесты на агрегацию (organic vs donate vs admin_refund). | ⚪ бэклог | 1.6.3 |
| **1.6.D** | Use-case `progression.add_length(player_id, delta, *, source, reason)` — единая точка прибавки длины. Внутри: проверка `anticheat_ban_until` → `AnticheatSoftBanError` если активен. Иначе clamp по `daily_cap_cm` и `weekly_cap_cm`, запись в `audit_log` с фактической дельтой и `clamped_from`. Trip-wire после mutation: рекомпьют → если суммы > лимита → `anticheat_ban_until = now + 14d`, action `ANTICHEAT_*_CAP_EXCEEDED`, alert админу через `INotifier`. Полный набор юнит-тестов + race-test (10×100 параллельных коллов). | ⚪ бэклог | 1.6.4 |
| **1.6.E** | Гейт `AnticheatGuard` для всех «спендалок» длины (`/upgrade`, в будущем `/duel`/караваны/рейды) — если `anticheat_ban_until > now()` → ошибка «вы в режиме проверки». Подключение к существующему `UpgradeThickness`. Юнит-тесты на каждый гейт. | ⚪ бэклог | 1.6.5 |
| **1.6.F** | Миграция всех существующих use-cases (`FinishForestRun`, `InvokeOracle`, `RegisterPlayer` с реферальным бонусом) на вызов `progression.add_length(...)` через DI-порт `ILengthGranter`. Отказ от прямых `player.with_length(...)` + `repo.save(player)`. Добавление `import-linter`-контракта «прибавка длины только через ILengthGranter». Регрессионные тесты. | ⚪ бэклог | 1.6.6 |
| **1.6.G** | Bot-команда `/anticheat_unban <tg_id> <reason>` (только `super_admin`, обязательная причина) — снимает `anticheat_ban_until`, пишет `admin_audit_log`. Локализация (`anticheat-soft-ban-active`, `anticheat-cap-clamped[-daily/-weekly]`, `anticheat-admin-alert`, `anticheat-unban-success`/`anticheat-unban-not-banned`/`anticheat-unban-not-authorized`). Юнит-тесты на authz и handler. | ⚪ бэклог | 1.6.7, 1.6.8 |
| **1.6.H** | Интеграционный нагрузочный тест: 100 параллельных лесов одного игрока → суточная сумма ≤ 3000, ни одна транзакция не «прорывает» лимит. Документация: README + `docs/anticheat.md` (как добавить новый source, как вручную снять бан). | ⚪ бэклог | 1.6.9, 1.6.10 |

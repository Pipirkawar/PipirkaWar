...
**Активная feature-ветка:** `devin/1778267195-sprint-3-3-D-boss-handlers-ui` от `main = d08985e` (merge 3.3-C). Открытых PR-ов на этой ветке пока нет — старт Спринта 3.3-D, шаг **D.0/D.15** (этот коммит).
...
- [x] **D.0 — Обновить `current_tasks.md`** под старт Спринта 3.3-D (этот коммит): «Снимок состояния» пересобран под `main = d08985e` (merge 3.3-C), чек-лист передвинут на 3.3-D, секция «Что ровно сейчас в работе» переписана под старт.
...
**Текущий PR — 3.3-D (UI + handler-ы + APScheduler factory-wiring + закрытие Спринта 3.3 «Рейд-боссы»):** ⏳ старт (этот коммит — D.0).
- **Текущий шаг:** D.0/D.15 — пересобран `current_tasks.md` под старт 3.3-D. Дальше: D.1 (use-case `CancelBossFight` + DTO), D.10 (его unit-тесты), D.2 (raider-loss length-вычеты в `FinishBossFight`), D.3 (APScheduler-фабрики), D.6 (локали `bosses.ftl` RU+EN), D.5 (`BossPresenter`), D.4 (handler `/boss`), D.7 (нотификаторы), D.8 (DI), D.9 (checkpoint push), D.11/D.12 (unit + integration), D.14 (`make ci`), D.15 (финальные доки).

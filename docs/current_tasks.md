...
- [ ] `git fetch && git checkout main && git pull` (свежий `main = <hash после merge PR #128>`).
- [ ] Создать ветку `devin/<timestamp>-sprint-4-1-B-prize-pool` от свежего `main`.
- [ ] **B.0 — Обновить `current_tasks.md`** под старт Спринта 4.1-B: пересобрать «Снимок состояния» под актуальный `main = <merge-hash>`, расписать чек-лист 4.1-B, заархивировать чек-лист 4.1-A (Checkpoint 1).
- [ ] **B.1 — Domain prize pool** (`src/pipirik_wars/domain/monetization/`): расширить `entities.py` агрегатом `PrizePool(stars: StarsAmount, ton_nano: TonNanoAmount, usdt_decimal: UsdtDecimalAmount)` с invariant-ами `>= 0`; новые VO `TonNanoAmount(int, >= 0)` + `UsdtDecimalAmount(int, >= 0)` (чтобы держать `Currency.TON_NANO` / `Currency.USDT_DECIMAL` строго типизированными); метод-фабрика `PrizePool.empty()`; метод `apply_increment(currency: Currency, amount_native: int) -> PrizePool` (возвращает новый `PrizePool` с увеличенным balance в нужной валюте — иммутабельно); ошибка `PrizePoolAmountInvariantError` при попытке убавить ниже нуля. 12+ unit-тестов agg-а + VO.
- [ ] **B.2 — Application use-case `RecordDonation`** (`src/pipirik_wars/application/monetization/record_donation.py`): принимает `(player_id, currency, amount_native, idempotency_key) -> RecordDonationResult` (`donation_idempotent: bool`, `pool_after: PrizePool`); внутри: атомарно `apply_increment(currency, amount_native // 10)` через `IPrizePoolRepository.apply_increment(...)`; идемпотентность — DB-side `UNIQUE`-индекс по `idempotency_key` для записи в `prize_pool_audit` или альтернативный механизм dedup; audit-source `prize_pool_increment` per-донат. 8+ unit-тестов use-case-а: успешный 100⭐→+10⭐, повторный idempotency_key (noop), 0-value донат (фильтруется), все 3 валюты.
...
`<обновится после A.8 push>` — `docs(4.1-A): A.8 — финальный док-коммит (history.md +1, current_tasks.md sync под 4.1-B)`.

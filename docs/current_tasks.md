...
- [ ] **2.5-D.1 — `/clan <id|name>`** — read-only карточка клана (как `/player` для игроков): id, name, owner, размер, сум-длина, last-active-clan-attack, статус (active / frozen + reason), список последних 5 daily-head-результатов. Use-case `GetClanCard(admin_id, query)`. Без TOTP. Audit: `ADMIN_CLAN_LOOKUP`. Локаль `admin-clan-*`.
- [ ] **2.5-D.2 — `/freeze_clan <id> [reason]` / `/unfreeze_clan <id>`** — обратимая блокировка клана (без TOTP, как `/freeze` для игроков). Use-cases `FreezeClan` / `UnfreezeClan` через новые `IClanRepository.freeze` / `unfreeze` (или `Clan.freeze(now, reason)` если решим хранить статус как enum). Идемпотентно. Audit: `ADMIN_CLAN_FROZEN` / `ADMIN_CLAN_UNFROZEN`. Локали `admin-freeze-clan-*` / `admin-unfreeze-clan-*`.
- [ ] **2.5-D.3 — `/clan_daily_head_history <id>`** — история daily-head (последние N=10): дата, head, score. Use-case `GetClanDailyHeadHistory(admin_id, clan_id, limit=10)` через новый read-метод `IDailyHeadRepository.list_recent_for_clan(clan_id, limit)`. Без TOTP. Audit: `ADMIN_CLAN_LOOKUP` (тот же ключ — это read-операция по клану). Локаль `admin-clan-daily-head-history-*`.
...
- [ ] **2.5-D.5 — `/audit [admin_tg_id] [action] [N]`** — query последних N записей `admin_audit_log` с опциональными фильтрами по `admin_id` и `action`. Use-case `GetAdminAuditTrail(admin_id, *, target_admin_tg_id?, action?, limit=50)` через новый read-метод `IAdminAuditLogger.list_recent(...)` (или отдельный `IAdminAuditQuery`-порт). Read-only, без TOTP. Сам audit `ADMIN_AUDIT_QUERIED` (читать аудит — тоже событие). Локаль `admin-audit-*`.
...
- **RBAC** (`Admin.role` enum + `IAdminAuthorizationPolicy.can(admin, command_kind)`) — это структурное изменение, затрагивает все админ-команды (включая старые из 2.5-B/C/legacy). Самый рискованный пункт спринта; стоит отдельным первым коммитом, чтобы можно было его откатить если что-то сломается.

...
**Закрыт Спринт 3.1 «PvE-Expeditions»** (5 PR-ов: 3.1-A → 3.1-E + catch-up #106). **Закрыт Спринт 3.2 «Караваны (полная механика)»** (4 PR-а: 3.2-A → 3.2-D). **Активный — Спринт 3.3 «Рейд-боссы»** ([`development_plan.md`](development_plan.md) §6.3.3); закрыты 3 PR-а из 4 (3.3-A + 3.3-B + 3.3-C), в работе — 3.3-D (закрытие Спринта 3.3).

**Активная feature-ветка:** `devin/1778267195-sprint-3-3-D-boss-handlers-ui` от `main = d08985e` (merge 3.3-C). Открытых PR-ов на этой ветке пока нет — Спринт 3.3-D, шаг **D.5/D.15 завершён → дальше D.4**. На ветке уже закоммичены D.0 (`a8a981a`), D.1+D.10 (`baf4d50`), D.2 (`236dd16`), D.3 (`866cc9b`), D.6 (`b1f6ff1`), D.5 (`bb31054`).
...
- [x] **D.5 — `BossPresenter`** (`bot/presenters/bosses.py`, коммит `bb31054`): рендер `BossFightSnapshot` в текст для лобби (имя саммонера + список рейдеров + таймер до начала) + рендер для боя (текущая длина босса / список рейдеров с урон-контрибьюцией / номер раунда / таймер до следующего tick) + рендер исхода (победа: список рейдеров с length-grant-ами + scroll-drop-ами; поражение: длина босса + raider-loss-вычеты).
- [x] **D.6 — Локали `bosses-*`** (`locales/{ru,en}.ftl`, коммит `b1f6ff1`): переводы лобби-UI / round-tick-нотификаций / fight-finish-нотификаций / scroll-drop-уведомлений / cancel-уведомления / error-сообщений (`BossSummonCooldownActive` / `BossPlayerLengthInsufficient` / `BossLevelInsufficient` / `BossLobbyFull` / `AlreadyInBossLobby` / `NotAuthorizedToCancelBoss` / etc.). RU+EN parity (тест `tests/unit/i18n/test_locale_parity.py` на parity ключей).
...
- [x] **D.10 — Юнит-тесты use-case-а `CancelBossFight`** (`tests/unit/application/bosses/test_cancel_boss_fight.py`, объединено с D.1 в коммите `baf4d50`: 11 тестов).
...
**Текущий PR — 3.3-D (UI + handler-ы + APScheduler factory-wiring + закрытие Спринта 3.3 «Рейд-боссы»):** ⏳ в работе. Закоммичено: D.0 (`a8a981a`), D.1+D.10 (`baf4d50`), D.2 (`236dd16`), D.3 (`866cc9b`), D.6 (`b1f6ff1`), D.5 (`bb31054`). PR в `main` пока не открыт.
- **Текущий шаг:** D.5/D.15 завершён → дальше **D.4** (handler `/boss` в `bot/handlers/boss.py` по образцу `bot/handlers/caravan.py`: команда `/boss` в личке, `SummonBoss` use-case, объявление в чате с inline-кнопкой «Показать лобби», callback-роутер `boss:show_lobby/join/leave/cancel` с использованием уже готового `BossPresenter` (D.5) и локалей `bosses-*` (D.6)). Затем D.7 (boss-нотификаторы round-tick / fight-finish), D.8 (DI: `CancelBossFight` в Container + factory-callbacks с реальными use-case-ами + нотификаторы), D.9 (checkpoint push), D.11/D.12 (unit + integration), D.14 (`make ci`), D.15 (финальные доки).
- **Согласование raider-loss формулы (D.2):** на старте принят вариант `Δ = max(0, length_at_join_cm - current_length_cm)` (рейдер теряет ровно то, что у него уже снял босс через damage в раундах). Опционального `bosses.raider_loss_floor_cm` НЕ вводим — для рейдеров без урона (живые без damage по формуле блокировок) `Δ=0`, что эквивалентно «выжил, но потерял время — ничего не потерял по длине». Тонкость: при поражении (`current_boss_length_cm >= victory_threshold_cm`) забираем у каждого рейдера ровно `Δ` через `progression.add_length(player_id=raider, delta_cm=-Δ, idempotency_key='boss_fight_raider_loss:{id}:{player}')`; зеркальная сумма уже учтена в boss-grant-е (boss получил `+sum(length_at_join_cm)`, а здесь рейдер «отдаёт» только реально потерянную в бою часть — система не претендует на снятие длины, которой у рейдера уже нет).
...
`bb31054` — `feat(3.3-D): BossPresenter — locale-driven /boss UI rendering (D.5)`.

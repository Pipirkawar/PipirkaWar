...
**На `main`:** последний смерженный PR — **3.3-C** (PR #114, `d08985e`); поверх него готовится к мерджу **PR Спринта 3.3-D** (текущая ветка `devin/1778267195-sprint-3-3-D-boss-handlers-ui`) — финальный PR Спринта 3.3 «Рейд-боссы»: bot-handler `/boss` + lobby-UI с inline-кнопками (`boss:show_lobby/join/leave/cancel`) + презентер `BossPresenter` + локали `bosses-*` (RU+EN parity, 39 ключей × 2 языка) + APScheduler-фабрики (`boss_lobby_close_factory` / `boss_round_tick_factory` / `boss_fight_finish_factory`, до этого `factory=None` с 3.3-B) + 3 Telegram-нотификатора (`TelegramBossLobbyCloseNotifier` / `TelegramBossRoundTickNotifier` / `TelegramBossFightFinishNotifier`, best-effort TG-publish, идемпотентность через `was_already_*`-флаги) + use-case `CancelBossFight` (саммонер отменяет рейд из лобби, идемпотентно, audit `BOSS_FIGHT_CANCELLED`) + raider-loss length-вычеты при поражении (`Δ = max(0, length_at_join_cm - current_length_cm)`, idempotency-key `boss_fight_raider_loss:{id}:{player}`, отложен из 3.3-C) + integration-тест частот scroll-drop-а (`tests/integration/application/bosses/test_scroll_drop_frequencies.py`, 100 рейдов × 5 рейдеров = 500 трайлов на каждый тип, 3σ-Bernoulli-bounds + ±10 floor для `blessed`, отложен из 3.3-C). +153 unit-теста (47 handler / 35 notifiers / 71 presenter+parity); total `make ci`: **4485 passed / 2 skipped, coverage 95.43%**. Manual smoke-тест в Telegram (D.13) — N/A в фоновой агент-сессии (требует живого `BOT_TOKEN`); функциональный equivalence покрыт unit/integration-тестами.

Перед ним: **3.3-C** (PR #114, `d08985e`) — доменный сервис `boss_round_resolution` + use-case-ы `RunBossRound` / `FinishBossFight` (per-player length-grant + независимый ролл `regular`/`blessed`-скроллов audit-only, boss victory-clamp до `victory_threshold_cm`). Перед ним: **3.3-B** (PR #113, `9c859b7`) — use-case-ы `SummonBoss` / `JoinBossLobby` / `LeaveBossLobby` / `CloseBossLobby` + миграция `0020_boss_fights` + APScheduler с `factory=None`-заглушками + 8 `AuditAction.BOSS_*`. Перед ним: **3.3-A** (PR #112, `dbb9b1c`) — каркас доменов «Рейд-босс». Перед ним: **3.2-D** (PR #111, `89e4f0a`) — bot-handlers `/caravan` + lobby-UI + закрытие Спринта 3.2. Перед ним: **3.2-C** (PR #110, `2333297`) — боевая механика каравана. Перед ним: **3.2-B** (PR #109, `e27968b`) — use-case-ы каравана + миграция `0019_caravans`. Перед ним: **3.2-A** (PR #108, `fe959c6`) — каркас доменов «Караван». Перед ним: **3.1-E** (PR #107, `5c1b26f`) — закрытие Спринта 3.1. Перед ним: PR-ы Спринта 3.1 (#99–#106) и Спринта 2.5 (#79–#97).

**Закрыт Спринт 3.1 «PvE-Expeditions»** (5 PR-ов: 3.1-A → 3.1-E + catch-up #106). **Закрыт Спринт 3.2 «Караваны (полная механика)»** (4 PR-а: 3.2-A → 3.2-D). **Закрывается Спринт 3.3 «Рейд-боссы»** ([`development_plan.md`](development_plan.md) §6.3.3) — 4 PR-а (3.3-A + 3.3-B + 3.3-C + 3.3-D); **3.3-D — текущий PR в работе**. После мерджа `3.3-D` активным становится **Спринт 3.4 «Заточка предметов»** ([`development_plan.md`](development_plan.md) §6.3.4).

**Активная feature-ветка:** `devin/1778267195-sprint-3-3-D-boss-handlers-ui` от `main = d08985e` (merge 3.3-C). PR откроется после финального D.15-доккоммита (этого). На ветке закоммичено: D.0 (`a8a981a`), D.1+D.10 (`baf4d50`), D.2 (`236dd16`), D.3 (`866cc9b`), D.6 (`b1f6ff1`), D.5 (`bb31054`), checkpoint-доки (`56fd4c2`), D.4 (`36cabdb`), D.7+D.8 (`479cdd3`), D.11a (`ddf08da`), D.11b (`36ff495`), D.11c (`a1e7b75`), mypy-fix (`534ecf4`), D.12 (`2bbb9fc`).
...
**Текущий PR — 3.3-D (UI + handler-ы + APScheduler factory-wiring + закрытие Спринта 3.3 «Рейд-боссы»):** ⏳ финальный док-коммит D.15 (этот) → PR в `main` → ожидание зелёного GitHub CI → squash-merge. На ветке закоммичены **все** 14 шагов чек-листа 3.3-D (D.0–D.12, D.14, D.15 + checkpoint-доки + mypy-fix). D.13 (manual smoke-тест в Telegram) — N/A в фоновой агент-сессии (требует живого `BOT_TOKEN`); функциональный equivalence покрыт 153 unit/integration-тестами и зелёным `make ci` (4485 passed / 2 skipped, coverage 95.43%).
- **Текущий шаг:** D.15 (этот) — `history.md` + запись 3.3-D + закрытие Спринта 3.3 + пересборка `current_tasks.md` под старт Спринта 3.4 «Заточка предметов».
- **Открытые блокеры:** нет.

После мерджа 3.3-D — следующий агент стартует **3.4-A** по чек-листу выше.

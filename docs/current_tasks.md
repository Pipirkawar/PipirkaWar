...
**`make ci` локально на feature-ветке (после D.1+D.2+D.3+D.5+D.9):** зелёный — 3219 passed / 1 skipped, coverage 95.98%.
...
| **Последний коммит на feature-ветке** | `176216c` Sprint 2.5-D.3: /clan_daily_head_history (read-only) |
| **PR (если открыт)** | будет открыт после промежуточной серии D.1+D.2+D.3+D.5+D.9; D.4/D.6/D.7+D.8/D.10–D.12 — отдельной серией коммитов на той же ветке (ребейз/append) |
| **CI статус** | зелёный локально на feature-ветке (после D.1+D.2+D.3+D.5+D.9): `make ci` — 3219 passed / 1 skipped, coverage 95.98% |

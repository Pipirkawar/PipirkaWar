...
- [x] **B.2 — Application use-case `RecordDonation`** (`src/pipirik_wars/application/monetization/record_donation.py`): принимает `(currency, payment_amount_native, idempotency_key) -> RecordDonationResult` (`donation_amount_native: int`, `pool_after: PrizePool`, `applied: bool`); вычисляет `donation = payment_amount_native // 10` (`floor`-округление, ГДД §12.6.1) и вызывает `repo.apply_increment(currency, donation)` при `donation > 0`, иначе возвращает текущий снапшот через `repo.get_current()` (`applied=False`). Идемпотентность наследуется от caller-а (`SpinPaidRoulette` в B.5 сам идемпотентен). audit-запись и `AuditSource.PRIZE_POOL_INCREMENT` добавятся в B.4 вместе с Alembic-миграцией whitelist-а. 14 unit-тестов: floor-округление (3), все 3 валюты (1 параметризованный, 3 кейса), result-shape (3), 0-фильтр (3), накопление (1), изоляция валют (1), command-shape (2). Checkpoint sha `f57008f`.
- [x] **B.3 — Persistence** (`src/pipirik_wars/infrastructure/db/`): таблица `prize_pool_balance` (`id BIGSERIAL`, `currency VARCHAR(16) UNIQUE NOT NULL CHECK (...)`, `balance_native NUMERIC(38,0) NOT NULL CHECK >= 0`, `updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`) + Alembic-миграция `0027_prize_pool_balance` с initial-seed-row на каждую `Currency` (3 строки `(stars|ton_nano|usdt_decimal, 0)`); ORM `PrizePoolBalanceORM` + `SqlAlchemyPrizePoolRepository` имплементация порта (`get_current()` → `SELECT currency, balance_native` → `_assemble_pool(...)`; `apply_increment()` → `UPDATE prize_pool_balance SET balance_native += :delta, updated_at = :now WHERE currency = :c` + повторный `SELECT` для свежего снапшота). 13 integration-тестов: `get_current` (2), `apply_increment` round-trip × 3 валюты + currency-isolation + накопление + `updated_at` (5), DB-CHECK invariants (3 — negative-balance, unknown-currency, duplicate-currency), existing-state round-trip (1). 3 теста миграций (revision, descend-from-0026, file-listing, table в smoke-таблицах). _Note:_ для 4.1-B остановиться на одной таблице — отдельный audit-trail добавится в B.4. Checkpoint 3 — push после commit-а.
- [x] **B.4 — Audit-source `prize_pool_increment`** (`src/pipirik_wars/domain/shared/ports/audit.py` + Alembic CHECK): в доменном енуме добавлены `AuditAction.PRIZE_POOL_INCREMENT` и `AuditSource.PRIZE_POOL_INCREMENT`. Новая Alembic-миграция `20260510_0028_audit_source_prize_pool_increment.py` расширяет `audit_log_source_whitelist` строкой `'prize_pool_increment'` (drop+create CHECK через `op.batch_alter_table` — SQLite-совместимо); downgrade — обратно к whitelist-у 0026. `RecordDonation` расширен через DI (`audit_logger: IAuditLogger`, `clock: IClock`): на `applied=True` пишет `AuditEntry` (action+source=`PRIZE_POOL_INCREMENT`, `actor_id=None`, `target_kind="prize_pool"`, `target_id=f"{cmd.key}:donation"`, `before=None`, `after={'currency','amount_native','pool_after_native'}`, `idempotency_key=f"{cmd.key}:prize_pool"`, `reason="prize_pool_increment"`); на `applied=False` — audit не пишется. 5 новых audit-тестов (`TestAuditWrite`) + 1 новый migration-тест (`test_0028_descends_from_0027`) + обновлен файл-реф в `test_audit_source.py` (0026 → 0028) и versions-listing. Локальный `make ci` зелёный: 5463 passed, mypy strict clean, ruff clean, 4/4 import-contracts.
- [x] **B.5 + B.6 — Интеграция `RecordDonation` в `SpinPaidRoulette`-flow + composition root** (объединены в один коммит ради сохранения зелёного `make ci`): `SpinPaidRoulette.__init__` расширен обязательным `record_donation: RecordDonation`; в `execute(...)` после Step 5 (audit `PAYMENT_RECORDED`) добавлен **Step 5b** — вызов `record_donation.execute(RecordDonationCommand(currency=Currency.STARS, payment_amount_native=cost_stars, idempotency_key=command.idempotency_key))` в той же UoW. `RecordDonation` сам формирует audit-`idempotency_key` со scope `:prize_pool` (отдельно от `:payment` Step-а 5). `floor`-округление в `RecordDonation` обеспечивает `donation == 0` для дефолтных `cost_stars_single=1 ⭐` / `cost_stars_pack10=9 ⭐` (no-op, audit не пишется). Composition root (`bot/main.py`): новые поля Container-а `prize_pool_repo: IPrizePoolRepository` + `record_donation: RecordDonation`, инстанцирование `SqlAlchemyPrizePoolRepository(uow=uow)` + `RecordDonation(prize_pool_repository=..., audit_logger=audit, clock=clock)`, проброс в `SpinPaidRoulette`. Тесты: 6 новых unit-тестов в `TestPrizePoolDonation` (1⭐ не инкрементирует пул, 9⭐ не инкрементирует, 100⭐ single инкрементирует на 10 + audit-запись с правильными ключами, 100⭐ pack-10 инкрементирует РОВНО ОДИН раз за весь pack — защита от N-кратного доната, idempotent-replay не делает повторный донат, gate-ошибка откатывает UoW и пул не изменяется); 14 существующих unpack-сайтов `_build_use_case` мигрированы под 9-tuple. `tests/unit/bot/test_composition_root.py` расширен для fakes + sqlalchemy-real сборки. `make ci` зелёный: 5469 passed, mypy strict 0/928, ruff clean, 4/4 import-contracts, coverage 95.53%.
- [ ] **B.7 — `make ci` локально:** ruff + `mypy --strict` + import-linter (4 contracts kept) + pytest зелёный + coverage gate (≥ 80%). [фактически уже зелёный на B.5+B.6 коммите; этот шаг — финальная верификация после B.8 док-коммита]
...
- [ ] `chore: remove AGENT_HANDOFF before PR` — отдельным коммитом удалить `AGENT_HANDOFF.md` (sticky-режим — выносим перед PR-ом).
...
`d2dff29` — `feat(4.1-B): B.4 — audit-source PRIZE_POOL_INCREMENT + Alembic 0028 + audit-write в RecordDonation`. Следующий коммит после этого обновления — B.5+B.6 интеграция `RecordDonation` в `SpinPaidRoulette` + composition root (строка переписывается тем же коммитом).

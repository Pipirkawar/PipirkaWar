...
- [x] **C.3 — Use-case `FinishBossFight`** (`application/bosses/finish_boss_fight.py`): rewards-механика. Победа рейдеров (`current_boss_length_cm < victory_threshold_cm`) — length-grant каждому живому рейдеру через `ILengthGranter` + per-player ролл скроллов заточки (обычный + blessed) через `IRandom` с idempotency-key `boss_scroll_drop:{boss_fight_id}:{player_id}:{scroll_kind}`; босс получает refund (если применимо). Поражение (timeout) — рейдеры теряют контрибьюцию, босс получает grant. Audit-actions `BOSS_FIGHT_FINISHED` + `BOSS_REWARDS_GRANTED` + N× `LENGTH_GRANT` + N× `SCROLL_DROP` (action добавится в C.0). Идемпотентность повторного вызова (повторный шедул APScheduler). + 17 unit-тестов (`test_finish_boss_fight.py`).
- [x] **C.4 — `application/dto/inputs.py`:** `RunBossRoundInput` (в C.2), `FinishBossFightInput` (в C.3).
- [x] **C.5 — Inventory/scroll-drop путь** — проверено: `domain/inventory/` ещё не существует; на 3.3-C — audit-only (запись `SCROLL_DROP` в `audit_log`); реальная инвентарная инфраструктура — Спринт 3.4 «Заточка предметов».
- [x] **C.6 — Audit-action `SCROLL_DROP`** (`domain/shared/ports/audit.py`) — добавлен в whitelist.
- [x] **C.7 — Архитектурный гард** (`tests/unit/architecture/test_length_grant_guard.py`): добавлен `application/bosses/finish_boss_fight.py` в whitelist для `Player.with_length()` (refund-к-самому-себе при victory-clamp до `victory_threshold_cm`; `ILengthGranter` для positive-grant-ов рейдерам/боссу).
- [x] **C.8 — DI-провязка** (`bot/main.py`): `RunBossRound` + `FinishBossFight` добавлены в `Container` (поля + инстанциирование в `build_container()`). Фикстура `_container_with_fakes()` в `tests/unit/bot/test_composition_root.py` обновлена (fake-инстансы + проброс в `Container(...)`). APScheduler-фабрики `boss_round_tick_factory` + `boss_fight_finish_factory` остаются `None` до 3.3-D.
- [x] **C.9 — Push checkpoint** на `origin` для непрерывности (после C.2 и после C.3+C.10).
- [x] **C.10 — Юнит-тесты use-case-ов** (`tests/unit/application/bosses/test_run_boss_round.py` + `test_finish_boss_fight.py`): happy-path (победа рейдеров / поражение / next-round-scheduled) + error-cases (boss_fight не найден, в `LOBBY` — нельзя запустить раунд, в `FINISHED` — идемпотентно). Integration-тест: 100 рейдов × 5 игроков с проверкой частот scroll-drop-а в границах толерантности (`scroll_drop.regular=0.05 ± δ`, `blessed=0.005 ± δ`) — **в работе** (после C.8).

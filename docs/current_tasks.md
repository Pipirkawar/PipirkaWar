...
| **2.1.B** | Доменный агрегат `Duel` — жизненный цикл боя 1×1: `DuelState` (PENDING_ACCEPT → IN_PROGRESS → COMPLETED/CANCELLED), `DuelMode` (CHAT_THEN_GLOBAL / CHAT_ONLY / GLOBAL_ONLY), `PendingRound` value-object. Lifecycle-методы (frozen-датакласс, immutable-замены): `Duel.create_challenge(...)` / `accept(...)` / `cancel(...)` / `submit_move(...)` / `force_complete_round(...)` (AFK-фоллбэк). Снэпшоты на старте — `hit_pct` и `expected_rounds` из баланса в момент вызова, `pX_initial_length_cm` в момент accept-а. Path-independent резолв на всех раундах. Без `random` в чистом домене (AFK-фоллбэки приходят снаружи как `RoundChoice`). Доменные ошибки `InvalidDuelStateError` / `NotADuelParticipantError` / `SelfChallengeError` / `MoveAlreadySubmittedError` / `NoMissingMovesError`. **76 новых unit-тестов**: lifecycle (47 — create / accept / cancel / properties / immutability), submit_move + полные 3-раундовые сценарии + force_complete + PendingRound (29). Без БД и use-cases — это в 2.1.C–D. | 🟡 готово к ревью | 2.1.6 (доменная часть activity lock & cooldown — поведение на агрегате) |
| **2.1.C** | Persistence: миграция `0009_pvp_duels` + ORM (`PvpDuelORM`, `PvpDuelRoundORM`) + порт `IDuelRepository` + реализация `SqlAlchemyDuelRepository` (load/save с lazy-coupled rounds). Сериализация enum-ов (`DuelState` / `DuelMode` / `Position`) в TEXT с CHECK-констрейнтами. Integration-тесты на miграцию (forward + downgrade) + roundtrip-репо. | ⚪ бэклог | 2.1.6 (БД-фундамент) |
| **2.1.D** | Use-cases боя: `ChallengeDuel` (создание вызова + activity-lock), `AcceptDuel` (приём + переход в IN_PROGRESS), `SubmitMove` (выбор атаки/блока через `Duel.submit_move`), `ResolveDuel` (auto-pick для AFK через `IRandom` + применение ±длины через `progression.add_length(source=PVP_REWARD)` с anti-cheat-cap-ом из 1.6). Activity-lock интеграция (нельзя одновременно в `/forest` и в дуэли — ГДД §3.2). Audit-логи `LENGTH_GRANT` (zero-sum обмен) + `PVP_DUEL_*`. Тесты на use-cases с FakeDuelRepository / FakeRandom / FakeClock. | ⚪ бэклог | 2.1.6 (cooldown + activity lock), часть 2.1.5 (расчёт ±длины) |
| **2.1.E** | Bot-handler-ы и presenter-ы: `/duel <opponent>`, callback-ы кнопок «Принять», 6 inline-кнопок выбора атаки/блока (`pvp-attack-{high,mid,low}` + `pvp-block-{high,mid,low}`). `DuelPresenter` поверх `IMessageBundle` + локали `duel-*` в RU+EN. Режимы вызова «Чат → Глобал» (по умолчанию), «Только чат», «Только глобал» (ГДД §7.1). | ⚪ бэклог | 2.1.2 |
| **2.1.F** | Глобальное лобби FIFO (PG-table или Redis-list, выбираем в начале PR-а): TTL=10 мин, авто-переход `chat → global` через 3 мин (APScheduler-job). | ⚪ бэклог | 2.1.3 |

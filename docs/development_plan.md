...
- [ ] Authz-проверки (уровень, длина, членство в племени, activity-lock) выполнены до операции.
...
│   ├── filters/             # фильтры по уровню/длине/племени
...
### Спринт 1.1 — Регистрация игрока и племени (1 неделя)
...
| 1.1.5 | Use-case `JoinClan` — игрок видится в чате племени **И** зарегистрирован в боте → авто-членство | Юнит-тесты: игрок без регистрации в ЛС → `clan_members` не создаётся, бот шлёт инструкцию |
| 1.1.6 | **Заморозка** племени при удалении бота из группы (статус `frozen`, не `archived` и не удаление) | Тест: бот кикнут → `status='frozen'`, история сохранена; повторное добавление → `status='active'` |
...
- [x] **Регистрация (Спринт 1.1)** — `/start` в группе регистрирует игрока и племя атомарно; `/start` в личке отказывает с подсказкой; миграция `chat_migrate_to`; bootstrap super-admin из `BOOTSTRAP_ADMIN_IDS`.
...
- [x] **Прокачка / предсказатель / топ (Спринт 1.4)** — `/upgrade` с защитой «не спустить ниже 20 см» (`InsufficientLengthError`); `/oracle` идемпотентен по `(player_id, date_msk)`; `/profile` (нік с титулом + длина + толщина + инвентарь + племя); `/top` (топ-10 по длине внутри племени).
...
1. Игрок добавляется в племя-чат → бот видит `chat_member` update.
...
9. Игрок пишет `/top` → видит топ-10 племя-чата по длине.
10. Игрок пишет `/lang en` → следующие сообщения от бота — на английском.

Все шаги покрыты unit + integration-тестами.

**Что НЕ входит в MVP** — анти-чит хардкап (Спринт 1.6, pre-Phase-2 gate); PvP 1×1 и масс-PvP, племенные механики, Глава племени дня, реферальная система — Фаза 2; полный админ-интерфейс — Спринт 2.5; горы, данжон, караваны, рейды — Фаза 3; монетизация (Stars / TON / USDT), Webhook вместо long-polling, Redis-кэш — Фаза 4.
...
### Спринт 2.2 — Масс-PvP и племенные механики

> Сама регистрация племени уже сделана в Спринте 1.1 (через добавление бота в группу).

| # | Задача | Критерий приёмки |
|---|---|---|
| 2.2.1 | Команда `/clantop` | Топ племён по сумме длин активных участников |
| 2.2.2 | Масс-PvP: вызов племя→племя, кулдаун 6 ч | Все участники с длиной ≥ 20 см автозаписываются |
| 2.2.3 | Игрок в обоих племенах — пропускает | Юнит-тест |
| 2.2.4 | Боевая механика N×M: 1 атака + 1 блок, случайные пары | Все удары разрешаются за один тик; ничья при 0 живых |
| 2.2.5 | Журнал племенных атак (история в карточке племени) | Тесты: события не теряются при сбое |

### Спринт 2.3 — Глава племени дня 👑
...
| # | Задача | Критерий приёмки |
|---|---|---|
| 2.3.1 | Доменный сервис `DailyHeadService.assign_or_get(clan_id, source)` + use-cases `RequestDailyHead` (button) и `RunDailyHeadCron` (cron). Через `IRandom` и `IClock` (`Europe/Moscow`-aware) | Юнит-тесты: при < 5 активных в племени — отказ; алгоритм избегания последних 3 при ≥ 10; повторный вызов в тех же сутках возвращает уже назначенного главу |
| 2.3.2 | Кнопка `🎲 Назначить главу дня` / команда `/clan_head` в чате племени | E2E: первый клик — назначение и публикация; второй в тех же сутках — тихое сообщение «уже назначен» |
| 2.3.3 | Cron APScheduler с **per-clan random_offset(0..24h)** от 00:00 МСК (детерминированный hash от `clan_id + date` для воспроизводимости тестов) | Тест: разные племена получают разные times; тот же `(clan_id, date)` всегда даёт тот же offset; cron не назначает повторно если кнопка уже сработала |
...
| 2.3.5 | Сообщение в чат племени с цитатой; начисление **`uniform(1, 20)` см** через `progression.add_length(reason="daily_head")` | E2E: сообщение пришло, длина увеличилась, есть запись в `audit_log` с `bonus_cm` в [1, 20] |
...
| 2.3.8 | Корректный пропуск **замороженных** (`frozen`) и архивированных племён | Замороженный племя не получает ни кнопочного, ни cron-триггера; после разморозки — получает |
...
| 2.4.6 | Еженедельные итоги (cron вс. 18:00 UTC) | Бот постит в каждый племя карточку из ГДД §13.3 |
...
| 2.5.6 | Команды по племенам: `/clan`, `/freeze_clan`, `/unfreeze_clan`, `/clan_daily_head_history` | Поиск работает по `chat_id` и подстроке названия |
...
#### 6.3.1+ Декомпозиция Спринта 3.1 на PR-ы

> Скоуп 3.1.1–3.1.5 выше — итоговый чек-лист спринта (что должно быть в `main` к концу). Ниже — план поэтапного выкатывания через несколько PR-ов; делается параллельно с реализацией, чтобы каждый PR был обозримым на ревью и ставил независимый CI-зелёный.
>
> **Обоснование разбиения.** Спринт 3.1 затрагивает 4 архитектурных слоя (domain → application → infrastructure → bot) и расширяет shared-конфиг (`balance.yaml`, items_catalog) + локали (RU+EN). Классический «один PR на спринт» дал бы +20 файлов и >2000 строк диффа — невозможно ревьюить и анализировать причины CI-падений. Вместо этого спринт делится на 5 фичевых PR-ов + postmerge-PR в каждом, по аналогии со Спринтом 2.5 (A→B→C→D.1–D.12).
>
> **Жёсткие правила декомпозиции** (применяются к каждому PR-у спринта):
> - На `main` после каждого PR-а — production-код в рабочем состоянии (никаких «полу-фич» с импортами, которые нельзя вызвать).
> - Доменный слой не зависит от ещё не написанного infrastructure/bot слоя — порты (Protocol/ABC) объявляются вместе с domain, реализации — позже.
> - Каждый PR — самодостаточный по тестам: unit-тесты на новый доменный/application-код в том же PR-е.
> - Локали добавляются вместе с handler-ом, который их зовёт (lint-тест локалей `tests/unit/locales/test_admin_keys_lint.py` из 2.5-D.12 ловит RU↔EN-дрейф; для не-`admin-*`-ключей расширим test после создания первого `mountains-*`/`dungeon-*` ключа).
> - Каждый PR закрывается postmerge-документационным PR-ом по канону Спринта 2.5 (sync `history.md` + `current_tasks.md`).

| PR | Тема | Скоуп | Файлы (ориентировочно) | Покрывает задачи |
|---|---|---|---|---|
| **3.1-A** | Каркас доменов гор и данжона + балансовый конфиг | Новые модули `domain/mountains/`, `domain/dungeon/` (по образцу `domain/forest/`, см. `src/pipirik_wars/domain/forest/`): entities (`MountainRun`, `DungeonRun`), `errors.py`, `repositories.py` (Protocol-ports `IMountainRunRepository`, `IDungeonRunRepository`). Расширение `config/balance.yaml` секциями `mountains` / `dungeon` с pydantic-схемами (`MountainsConfig`, `DungeonConfig`): cooldown-границы, ± outcomes (mountains: gain-branch + loss-branch; dungeon: то же), drop probability + slot/rarity weights. Доменный picker `pick_pve_outcome(*, location, weights, rng) -> PveOutcome` поддерживает оба знака; для леса остаётся обратно-совместимый `pick_forest_outcome` (или унификация — решается в ходе реализации). Юнит-тесты доменных инвариантов + 1000+ rolls. **Без** обработчиков, миграций, локалей, бот-handler-ов. | `src/pipirik_wars/domain/mountains/*` (новые), `src/pipirik_wars/domain/dungeon/*` (новые), `src/pipirik_wars/domain/pve/services.py` (общий picker — если унифицируем), `config/balance.yaml` (+secs `mountains`, `dungeon`), `src/pipirik_wars/infrastructure/balance/schemas.py` (pydantic-расширения), `tests/unit/domain/mountains/*`, `tests/unit/domain/dungeon/*`. | 3.1.1 (домен), 3.1.2 (домен), 3.1.5 (схемы) |
| **3.1-B** | Use-cases `Start*Run` / `Finish*Run` + persistence + миграция | По образцу `application/forest/`: use-cases `StartMountainRun`, `FinishMountainRun`, `StartDungeonRun`, `FinishDungeonRun`. Hardcap-канон через `progression.add_length(...)` для +-исходов; для −-исходов — прямая запись через `progression.subtract_length(...)` (или helper) с audit. Activity-lock интеграция (см. `application/forest/start_run.py`). Idempotency-key. Alembic-миграция `00XX_pve_runs_mountain_dungeon.py` (либо новая таблица, либо расширение существующей `pve_runs`). SQLAlchemy-модели + repo-impl. Round-trip integration-тесты. **Без** локалей, бот-handler-ов, drop-engine-расширения (drop-конфиг минимальный, тестируем через FakeDropEngine). | `src/pipirik_wars/application/{mountains,dungeon}/*` (новые), `src/pipirik_wars/infrastructure/db/models/pve.py` (расширение или новый), `src/pipirik_wars/infrastructure/db/migrations/versions/00XX_pve_runs.py`, `src/pipirik_wars/infrastructure/db/repositories/{mountain_run,dungeon_run}.py`, `tests/unit/application/{mountains,dungeon}/*`, `tests/integration/db/test_pve_runs.py`. | 3.1.1 (use-case + persistence), 3.1.2 (use-case + persistence) |
| **3.1-C** | Дроп оружия (`right_hand`, `left_hand`) — drop-engine + items_catalog | Расширить `items_catalog` в `config/balance.yaml` минимум 6+ позициями оружия (по 3+ на `right_hand` и `left_hand`, по редкостям `common`/`rare`/`epic`). Расширить pydantic-валидатор — слот может быть одним из 8 (было 6); добавить инвариант `≥ 1 предмет на каждый из 8 слотов`. Drop-engine: добавить `slot_weights` per-location (forest/mountains/dungeon — у каждой свой расклад; см. ГДД §2.6 «Веса дропа на руки vs шапка/тело/… — в `balance.yaml` (`drop.slots`)»). Поддержать множественный дроп `0..N` (forest/mountains: 0–1; dungeon: 0–3). Юнит-тесты: 1000+ rolls per location, частоты в ожидаемых границах; уникальность дроп-ID-шников за один прогон. | `config/balance.yaml` (+items, +drop.slots), `src/pipirik_wars/infrastructure/balance/schemas.py`, `src/pipirik_wars/domain/economy/*` (drop-engine расширение), `tests/unit/domain/economy/*`. | 3.1.4, 3.1.5 (items_catalog) |
| **3.1-D** | Дроп скроллов заточки (skeleton, без use-механики) | Domain VO `Scroll(category: ScrollCategory, blessed: bool)` (skeleton — полная impl механики использования скролла откладывается на Спринт 3.4). Расширить drop-engine исходами `scroll_drop_regular` / `scroll_drop_blessed` (mountains: только regular, очень-очень-редко; dungeon: regular очень-редко + blessed очень-очень-редко; см. ГДД §2.8.5). Конфиг `enchantment.scroll_drops_per_location` в `balance.yaml` с pydantic-валидацией (вероятности ∈ [0, 1], blessed запрещён в горах). Юнит-тесты: 1000+ rolls на каждую локацию, частоты в ожидаемых границах. **Без** механики применения скроллов (это 3.4). | `src/pipirik_wars/domain/economy/scroll.py` (новый, skeleton), `src/pipirik_wars/domain/economy/*` (drop-engine), `config/balance.yaml` (+enchantment.scroll_drops_per_location), `src/pipirik_wars/infrastructure/balance/schemas.py`, `tests/unit/domain/economy/test_scroll_drops.py`. | 3.1.3, 3.1.5 (enchantment skeleton) |
| **3.1-E** | Bot-handlers `/mountains`, `/dungeon` + презентеры + локализация | Тонкий aiogram-слой по образцу `bot/handlers/forest.py`: `bot/handlers/{mountains,dungeon}.py`, `bot/presenters/{mountains,dungeon}.py`, `bot/notifications/{mountains,dungeon}.py`. Локали `mountains-*` / `dungeon-*` (RU + EN, parity автомат). Карточка возврата — текст исхода + список дропа + кнопки «надеть/выбросить» × N (для данжона до 3 кнопок). Расширить lint-тест `tests/unit/locales/test_admin_keys_lint.py` (либо создать симметричный `test_pve_keys_lint.py`) для гарантии RU↔EN-симметрии новых ключей. Handler-тесты + manual smoke. | `src/pipirik_wars/bot/handlers/{mountains,dungeon}.py`, `src/pipirik_wars/bot/presenters/{mountains,dungeon}.py`, `src/pipirik_wars/bot/notifications/{mountains,dungeon}.py`, `locales/ru.ftl` (+ключи), `locales/en.ftl` (+ключи), `tests/unit/locales/*` (расширение/новый файл), `tests/unit/bot/handlers/*`. | 3.1.1 (UX), 3.1.2 (UX) |

> **Каждый из 3.1-A…3.1-E** идёт в `main` своим PR-ом + сразу за ним postmerge-PR (sync `docs/history.md` + `docs/current_tasks.md`), по аналогии со Спринтом 2.5. Закрытие Спринта 3.1 — мерж postmerge-PR-а после 3.1-E + добавление в `history.md` записи о закрытии спринта.
>
> **Возможные точки риска / решения:**
> - **Унификация vs. раздельные модули.** Лес сейчас живёт в `domain/forest/` отдельно. Горы/данжон с механикой ± делают унификацию под `domain/pve/{forest,mountains,dungeon}` оправданной (DRY). Решение принимается в PR-е 3.1-A с обновлением этого плана. Если унификация — выкатывается через миграционный PR, переносящий `domain/forest/` под `domain/pve/forest/`, **до** 3.1-A; этот ход НЕ считается за фичевый PR Спринта 3.1, а отдельный chore-PR.
> - **Drop-engine — общий или per-location.** Текущий `forest`-специфичный drop-engine придётся обобщить. Если это становится крупным рефакторингом — выделяется в отдельный chore-PR `refactor: generalize PvE drop engine` до 3.1-C.
> - **Скроллы в 3.1-D.** Domain VO `Scroll` объявляется минимально; полная инвентарная сущность + use-case применения — Спринт 3.4. До тех пор скроллы из дропа просто пишутся в `audit_log` как «дроп будущей механики» (или в отдельный лог). **Не** копятся в инвентаре игрока в Спринте 3.1 (иначе придётся реализовывать инвентарь скроллов сейчас, что выходит за скоуп).

### Спринт 3.2 — Караваны (полная механика)

| # | Задача | Критерий приёмки |
|---|---|---|
| 3.2.1 | Создание каравана (lvl 7+), задание вклада + племя-получателя (5 рандомных или ручной ввод) | Юнит-тесты на правило «после взноса ≥ 20 см» |
| 3.2.2 | Лобби 20 мин, кулдаун племени 12 ч | E2E: 5 караванщиков + ≤ 4× рейдеров + ≤ 2× защитников |
| 3.2.3 | Роли при двойном членстве (см. ГДД §9.4) | Юнит-таблица всех 5 случаев |
| 3.2.4 | Запрет рейдерства членам обоих племён | Юнит-тест |
...
| 3.2.6 | Завершение: победа/проигрыш, награды (×4 лидеру, ×3 караванщикам, ×1 защитникам, +1 см племени), Атаман | Все множители из `balance.yaml` |
...
### Спринт 3.6 — Бонус-за-племена в Предсказателе 🎯

> **Виральная мини-механика.** Расширение `/predict` (см. §11.1 ГДД): за каждое **активное** племя, в котором состоит игрок, начисляется `+1 см` к базовому `uniform(1, 20)`. Cap = `+131 см` за вызов (итого `/predict` ≤ `+151 см` вместе с базой). Активным считается племя со `status="active"`, **числом участников `> 3`**, где игрок — член (см. §1.1 ГДД). Anti-cheat: **отдельный лимит** (`source = "oracle_tribe_bonus"` НЕ входит в organic 24h/7d). Display: явная строка `+N см за племена` в результате `/predict`. Снапшот — **live** в момент вызова `/predict`.
>
> **Важно: рефакторинг «клан → племя» — только в документации** (этот ПД, ГДД, current_tasks, history). Доменные идентификаторы `domain/clan/*`, `application/clan/*`, table `clans`, локали `clan-*`, команды `/clan`, `/freeze_clan`, `/clantop` — **сохраняются как есть** как намеренный docs-only-рефакторинг (без миграции БД, без переименования файлов, без слома обратной совместимости команд).

| # | Задача | Критерий приёмки |
|---|---|---|
| 3.6.1 | Доменный запрос: метод `IClanRepository.count_active_for_player(player_id, *, min_tribe_size: int) -> int` (или симметричный application-сервис в `application/clan/services/`). Активным считается клан в `status='active'` (не `frozen`/`archived`) с **числом участников `> min_tribe_size`**, где игрок — член. | Юнит-тесты на каждый gate (frozen → 0; size=3 при min=4 → 0; size=4 → +1; not_member → 0; bot-only-member → 0); mypy `--strict` зелёный |
| 3.6.2 | Расширение pydantic-схемы `OracleConfig` секцией `tribe_bonus` (`enabled: bool`, `cm_per_tribe: int >= 0`, `cap_cm: int >= 0`, `min_tribe_size: int >= 1`) с дефолтами по ГДД §11.1: `enabled=true`, `cm_per_tribe=1`, `cap_cm=131`, `min_tribe_size=4`. Инвариант `cap_cm + bonus_max <= 151` — мягкая sanity-check проверка (warning в логе). | Юнит-тесты pydantic-валидаторов; интеграционный тест: дефолтный `balance.yaml` парсится без ошибок |
| 3.6.3 | Application use-case `RequestOracle` — расширение: после рандома `uniform(bonus_min, bonus_max)` считаем `n_active_tribes = clan_repo.count_active_for_player(player_id, min_tribe_size=cfg.min_tribe_size)`, прибавляем `min(n_active_tribes * cm_per_tribe, cap_cm)`. **Два отдельных вызова** `progression.add_length(...)`: (1) `reason="oracle_base"`, `source="oracle"`; (2) `reason="oracle_tribe_bonus"`, `source="oracle_tribe_bonus"`. Обе проводки внутри одной транзакции и одного idempotency-key `oracle:{player_id}:{moscow_date}`. | Юнит-тесты: 0 племён → нет второй проводки; 5 племён → +5 см второй проводкой; 200 племён → +131 см (cap); idempotency повторного вызова за те же сутки → не дублирует ни одну проводку |
| 3.6.4 | Anti-cheat (см. ГДД §3.3.4 / §3.3.5): добавить `oracle_tribe_bonus` в **новый** whitelist `tribe_bonus_sources` в `balance.yaml::anticheat`. **НЕ** добавлять в `organic_sources`. Расширить `IAnticheatChecker` так, чтобы при агрегации rolling-окна источники из `tribe_bonus_sources` игнорировались. | Integration-тест: `5 × /predict с +131 см бонуса` подряд = `+655 см «в обход» хардкапа` за час → trip-wire `ANTICHEAT_DAILY_CAP_EXCEEDED` **не** срабатывает; audit-проводки с правильным `source` |
| 3.6.5 | Bot-presenter `OraclePresenter`: рендер сообщения `/predict` — **две** строки прироста (`+N см — базовый бонус` + `+M см — за племена (K активных племён)`) + итоговая строка `+(N+M) см — итого`. При `n_active_tribes == 0` — строка `за племена` **скрывается**, показывается только базовая. Опциональный hint `*вступай в новые племена и получай больше см*` — после первых 3 нулевых `/predict` подряд (feature-flag, по умолчанию off). | Снэпшот-тесты презентера на 4 сценария (0 / 1 / N / cap племён); locale-parity RU+EN |
| 3.6.6 | Локализация: ключи `oracle-base-line`, `oracle-tribe-bonus-line` (включая Fluent-плюрал-форму `1 племя / 2-4 племени / 5+ племён`), `oracle-total-line`, `oracle-no-tribes-hint` (опц.) — в обоих файлах `locales/ru.ftl` и `locales/en.ftl`. | Все ключи присутствуют в обоих файлах; e2e-snapshot RU+EN |
| 3.6.7 | Manual smoke в Telegram (не блокирует PR; делается на стейдже): `/predict` в чате/ЛС с 0 / 1 / N племенами показывает корректную строку. | Чек-лист в PR-описании, выполняется ревьюером после мерджа |
| 3.6.8 | Финальный док-коммит: `history.md` — запись о закрытии Спринта 3.6; `current_tasks.md` — пересборка под следующий спринт; `game_design.md` §11.1 — пометка «реализовано в Спринте 3.6 (PR #...)». | Docs-only-коммит после зелёного CI |

#### 6.3.6+ Декомпозиция Спринта 3.6 на PR-ы

> Спринт 3.6 — **компактный** (1–2 PR-а), потому что доменный слой кланов / племён (`domain/clan/*`, `application/clan/*`) уже существует с Фазы 1–2, а application use-case `RequestOracle` уже есть. Расширения минимальны.

| PR | Тема | Скоуп | Покрывает |
|---|---|---|---|
| **3.6-A** | Доменный запрос + конфиг + use-case + anti-cheat | `IClanRepository.count_active_for_player`, pydantic `OracleTribeBonusConfig`, расширение `RequestOracle` use-case-а (две проводки `add_length`), `tribe_bonus_sources` в `balance.yaml` + `IAnticheatChecker`. Юнит + integration-тесты. **Без** UI-изменений. | 3.6.1, 3.6.2, 3.6.3, 3.6.4 |
| **3.6-B** | Bot UI + локали + закрытие Спринта 3.6 | Расширение `OraclePresenter` строками `+N за племена`, локали RU+EN с Fluent-плюрал-формами, snapshot-тесты, manual smoke. + финальный док-коммит. | 3.6.5, 3.6.6, 3.6.7, 3.6.8 |
...
| 4.5.6 | Раздел «Племена»: карточка, история «Главы племени дня», заморозка/разморозка | Замороженный племя виден отдельным фильтром; история не теряется |
...
### Спринт 5.0 — Полный аудит проекта 🔍

> Проводится перед бета-тестированием. Цель — найти и исправить все баги, логические ошибки, пропущенные тесты; составить отчёт для ручного ревью владельцем.

| # | Задача | Критерий приёмки |
|---|---|---|
| 5.0.1 | Аудит Domain Layer (134 файла): player, clan, forest, pvp, monetization, balance, anticheat, etc. | Все найденные баги исправлены, тесты добавлены, отчёт `domain_audit.md` |
| 5.0.2 | Аудит Application Layer (163 файла): use-cases, RBAC, payments, caravans, bosses, etc. | Баги исправлены, edge-cases протестированы, отчёт `application_audit.md` |
| 5.0.3 | Аудит Infrastructure Layer (185 файлов): DB, Redis, payments, i18n, templates | SQL-инъекции, утечки, CVE проверены, отчёт `infrastructure_audit.md` |
| 5.0.4 | Аудит Bot Layer (91 файл): handlers, middleware, Composition Root (main.py) | Wiring проверен, graceful shutdown, rate-limiting, отчёт `bot_audit.md` |
| 5.0.5 | Аудит Admin Web + Config + Docs + CI: безопасность, актуальность документации | CSRF/XSS/TOTP проверены, docs↔код синхронизированы, отчёт `admin_web_config_audit.md` |
| 5.0.6 | Сводный аудит-отчёт (3 части): исправленные ошибки / требует внимания владельца / описание модулей | Единый файл `docs/audit_report.md` для ручного ревью |

### Спринт 5.1 — Совместный аудит с владельцем 🤝

> Владелец читает аудит-отчёт, принимает решения по спорным вопросам, правит бизнес-логику.

| # | Задача | Критерий приёмки |
|---|---|---|
| 5.1.1 | Ревью Части 1 отчёта (исправленные ошибки) — подтверждение что фиксы корректны | Владелец подтвердил все PR с фиксами |
| 5.1.2 | Ревью Части 2 отчёта (требует внимания) — решение по каждому пункту | Каждый пункт имеет решение: fix / wontfix / deferred |
| 5.1.3 | Ревью Части 3 отчёта (описание модулей) — владелец понимает всю логику | Владелец подтвердил что логика работы каждого модуля соответствует ожиданиям |
| 5.1.4 | Исправления по результатам совместного аудита | Все fix-пункты реализованы |

### Спринт 5.2 — Подготовка к бета-тестированию 🚀

> После аудита — настройка инфраструктуры и запуск бета-теста.

| # | Задача | Критерий приёмки |
|---|---|---|
| 5.2.1 | Настройка VPS: Docker Compose, PostgreSQL, Redis, Nginx reverse proxy | Бот запускается на VPS, отвечает на /start |
| 5.2.2 | Настройка мониторинга: Prometheus + Grafana дашборды | Метрики собираются, дашборды отображают DAU/RPS/ошибки |
| 5.2.3 | Настройка бэкапов: автоматический pg_dump + Neon point-in-time | Ежедневный бэкап, тестовый restore проведён |
| 5.2.4 | Настройка алёртов: бот не отвечает, DAU > 80%, ошибки платежей | Алёрты срабатывают в тестовом режиме |
| 5.2.5 | Настройка CI/CD: GitHub Actions → Docker → VPS автодеплой | Push в main → автоматический деплой на VPS |
| 5.2.6 | Создание Telegram-бота для бета-теста (отдельный @username) | Бот зарегистрирован, токен в секретах |
| 5.2.7 | Seed-данные для бета-теста: тестовые игроки, племена, баланс | Бот работает с тестовыми данными, все команды работают |
| 5.2.8 | Smoke-тест: полный прогон всех пользовательских сценариев | Чек-лист: /start → /forest → /oracle → /duel → /top → /profile |
| 5.2.9 | Запуск закрытой бета (приглашение 10-20 тестеров) | Тестеры играют, баги трекаются |
...
- ✅ **Кик бота из чата племени** — племя в статус `frozen` (не `archived` и не удаление).
...
- ✅ **Бонус Главы племени дня** — `uniform(1, 20)` см (`balance.yaml: daily_head.bonus_min/bonus_max`).
- ✅ **Время Главы племени дня** — гибридный триггер (кнопка `/clan_head` ИЛИ фоновый cron с per-clan `random_offset(0..24h)` от 00:00 МСК). Распределяет нагрузку и добавляет элемент «кто первый дёрнет».
...
- ✅ **Каналы как племена** — отказ полностью; **канал-анонсы** — отдельный спринт **4.9** в самом конце Фазы 4 перед маркетинг-релизом.
- ✅ **Стиль цитат, доп. правила** — уместный мат разрешён (`mild_profanity=true`); запрещены: политика, межнацоскорбления, насилие, реклама, секс. Цитаты с матом помечаются тегом `profanity` для фильтра «детский режим племени» в будущем.
...
5. **GDPR / полное удаление** — `frozen` для племён и заморозка для игроков фиксируют состояние, но не удаляют PII. Нужна ли GDPR-функция «удалить полностью» в MVP?

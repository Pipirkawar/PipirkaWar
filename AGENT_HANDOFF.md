...
- **Активный шаг чек-листа:** **C.6.a** — audit-source/action `PRIZE_LOT_RESERVED` + Alembic-миграция `0032_audit_source_prize_lot_reserved` (расширение `audit_log_source_whitelist`). Это первый микрошаг пере-декомпозированного блока C.6 («резервирование лота»): самому резервированию в use-case-ах посвящены C.6.b/C.6.c/C.6.d (см. разбивку в `docs/current_tasks.md`). **Схема следует C.4-паттерну** (`AuditAction.PRIZE_LOT_REFUNDED` + Alembic 0031): расширить оба enum-а `AuditAction` и `AuditSource` в `domain/shared/ports/audit.py`, написать миграцию `20260511_0032_audit_source_prize_lot_reserved.py` (drop+create CHECK через `op.batch_alter_table`), обновить `tests/unit/domain/shared/ports/test_audit_source.py::_load_migration_whitelist` на 0032 + `tests/integration/db/test_migrations.py::test_0032_descends_from_0031`.
- **Готовы:** C.0 (snapshot pivot + sticky HANDOFF), C.1 (Domain `PrizeLot` aggregate + VO `FeeBufferAmount` + ports + errors + 67 unit-тестов), C.2 (Application use-case `GeneratePrizeLots` + 41 unit-тестов + Alembic 0029 audit-source `prize_lot_generated`), C.3 (Alembic `0030_prize_lots` + `PrizeLotORM` + `SqlAlchemyPrizeLotRepository` + 33 integration-тестов), C.4 (`AuditAction.PRIZE_LOT_REFUNDED` + `AuditSource.PRIZE_LOT_REFUNDED` + Alembic `0031_audit_source_prize_lot_refunded`), **C.5** (picker `active_lots: Sequence[PrizeLot]` + `RouletteOutcome.crypto_lot(lot_id)` + invariants `kind == CRYPTO_LOT ⇔ lot_id >= 1` + 25 новых unit-тестов + use-case-ы прокинуты с `active_lots=()` на C.5).
- **В работе:** —.
- **Дальше:** C.6.b — прокинуть `IPrizeLotRepository.list_active(currency)` в use-case-ах спинов перед picker-ом; C.6.c — хаппи-патх резервирования + audit; C.6.d — race-fallback (решение при распараллеливании спинов двух игроков, MVP — LengthGain-выпадение); C.6.e — integration-тест. Далее C.7—C.11 (см. `docs/current_tasks.md`). Все оставшиеся блоки разбиты на микрошаги `C.X.a / .b / .c / ...` (пере-декомпозиция 2026-05-11 под уменьшенные лимиты).

...
- **F.4.b** (этот коммит) — Application use-case `LinkWallet` расширен под phase-2 двухфазного flow + anti-replay. Изменения:
...
- Файл: следующий шаг — `src/pipirik_wars/infrastructure/payments/ton_connect/proof_parser.py` (F.5.a: JSON-deserializer `TonProof.from_json(...)` из wallet-response в доменный VO; парсит `ton_proof.proof.{timestamp,domain.value,payload,signature}` + `account.{address,publicKey,walletStateInit}`; бросает `TonProofMalformedError(reason, raw_len)` на любых schema-violation-ах).
- Что планировал дальше: F.5.a (TonProof-JSON-deserializer) → F.5.b (canonical-message-builder) → F.5.c (`TonConnectProductionVerifier`) → F.6.a (Alembic 0038) → F.6.b (`SqlAlchemyNonceStore`) → F.7 (composition root + config-flag) → F.8.a (`/link_wallet`-phase-1) → F.8.b (`/link_wallet_confirm`-phase-2) → F.8.c (локали RU/EN) → F.9 (smoke-test) → F.10 (`make ci` sanity, не отдельный коммит) → F.11 (doc-sync) → F.12 (remove HANDOFF + PR).
...
- Последний коммит: `(F.4.b — этот, sha будет известен после commit-а)` — `feat(4.1-F): F.4.b — extend LinkWallet consume_nonce + scope/nonce in Command`.
- Незакоммиченные изменения: да — `src/pipirik_wars/application/monetization/link_wallet.py`, `src/pipirik_wars/infrastructure/payments/ton_connect/{__init__.py,in_memory_nonce_store.py (новый)}`, `src/pipirik_wars/bot/{main.py,handlers/link_wallet.py}`, `tests/unit/application/monetization/test_link_wallet.py`, `tests/unit/bot/test_composition_root.py` + `AGENT_HANDOFF.md` (этот файл).
- CI прогонялся? Локально: `make ci` → 6736 passed + 2 skipped (8m43s, coverage 95.46%). `ruff check` + `mypy --strict` + `lint-imports` зелёные.

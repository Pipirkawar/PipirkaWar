...
- Последний коммит: `(F.2 — этот, sha будет известен после commit-а)` — `feat(4.1-F): F.2 — domain VO TonProof + TonConnectVerificationError taxonomy`.
- Незакоммиченные изменения: да — `src/pipirik_wars/domain/monetization/{value_objects.py,errors.py,__init__.py}` + `tests/unit/domain/monetization/test_ton_proof.py` (новый) + `AGENT_HANDOFF.md` (этот файл).
- CI прогонялся? Локально: `pytest tests/unit/domain/monetization/test_ton_proof.py -q --no-cov` → 112 passed; `pytest tests/unit/domain/monetization/ -q --no-cov` → 425 passed; `ruff check` + `ruff format --check` + `mypy --strict` + `lint-imports` зелёные.

...
- **Активный шаг чек-листа:** **C.7.b** — cron-entry для `GeneratePrizeLots` в `infrastructure/scheduler/aps.py`: 3 instances `IntervalTrigger(hours=1)` (STARS / TON_NANO / USDT_DECIMAL), каждый вызывает `GeneratePrizeLots.execute(GeneratePrizeLotsCommand(currency=..., idempotency_key=f"prize_lot_generator:cron:{currency}:{utcnow_isoformat_period_id}"))`. Без триггера от RecordDonation — это C.7.d. Юнит-тесты: scheduler-registration shape; cron-id уникален per currency. **Важно**: composition root в `bot/main.py` пока **не** создаёт `GeneratePrizeLots`; в C.7.b нужно собрать `GeneratePrizeLots(uow=..., prize_pool_repository=prize_pool_repo, prize_lot_repository=prize_lot_repo, fee_estimator=InMemoryFeeEstimator(), audit_logger=audit, idempotency=idempotency, clock=clock)` — все зависимости уже доступны в `Container`.
...
- **C.7.a** (готов в этом коммите): `infrastructure/fees/InMemoryFeeEstimator` — stateless константный `IFeeEstimator`-адаптер (STARS=0, TON_NANO=10_000_000 ≈ 0.01 TON ≈ P95 plain-TON-перевода, USDT_DECIMAL=200_000 ≈ 0.2 USDT-decimal ≈ буфер на TON-газ jetton-перевода). `target_amount_native` игнорируется (константная оценка); на 4.1-D появится `TonRpcFeeEstimator` c P95 за 7 дней через TON RPC, контракт порта остаётся прежним. TODO(balance) в коде указывает на ГДД §12.6.3. 8 unit-тестов в `tests/unit/infrastructure/fees/test_in_memory_fee_estimator.py` (3 константы + target-ignored + non-negative-int + idempotent + protocol-conformance + async-сигнатура). mypy --strict зелёный (941→945 source files, 0 issues), import-linter зелёный (4 contracts kept), pytest зелёный (5647→5657 passed / 2 skipped, coverage 95.59%).
- **В работе:** —.
- **Дальше:** C.7.b — cron 1×/час scheduler entry-point `GeneratePrizeLots` per currency + создание `GeneratePrizeLots`-инстанса в `bot/main.py::Container` с `InMemoryFeeEstimator()` (все остальные DI уже доступны: `uow`, `prize_pool_repo`, `prize_lot_repo`, `audit`, `idempotency`, `clock`); C.7.c — smoke-тест cron-flow; C.7.d — триггер после крупного донат-зачисления (хук в `RecordDonation` или signal-poll, см. открытый вопрос). Далее C.8—C.11 (см. `docs/current_tasks.md`).

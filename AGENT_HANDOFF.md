...
- **Активный шаг чек-листа:** **C.6.b** — в use-case-ах `SpinPaidRoulette` / `SpinFreeRoulette` добавить DI-параметр `prize_lot_repository: IPrizeLotRepository` в `__init__`, перед picker-ом вызывать `active_lots = await prize_lot_repository.list_active(currency)` и передавать в `pick_*_outcome(…, active_lots=active_lots)`. **Открытый вопрос C.6.b:** валюта лота для free-рулетки (ГДД §12.4.2/§12.5.2 — уточнить, какую валюту даёт free CRYPTO_LOT; вариант по умолчанию — `STARS`, тот же что в paid). Резервирование и audit — не в C.6.b, а в C.6.c.
- **Готовы:** C.0 (snapshot pivot + sticky HANDOFF), C.1 (Domain `PrizeLot` aggregate + VO `FeeBufferAmount` + ports + errors + 67 unit-тестов), C.2 (Application use-case `GeneratePrizeLots` + 41 unit-тестов + Alembic 0029 audit-source `prize_lot_generated`), C.3 (Alembic `0030_prize_lots` + `PrizeLotORM` + `SqlAlchemyPrizeLotRepository` + 33 integration-тестов), C.4 (`AuditAction.PRIZE_LOT_REFUNDED` + `AuditSource.PRIZE_LOT_REFUNDED` + Alembic `0031_audit_source_prize_lot_refunded`), C.5 (picker `active_lots: Sequence[PrizeLot]` + `RouletteOutcome.crypto_lot(lot_id)` + invariants `kind == CRYPTO_LOT ⇔ lot_id >= 1` + 25 новых unit-тестов + use-case-ы прокинуты с `active_lots=()` на C.5), **C.6.a** (`AuditAction.PRIZE_LOT_RESERVED` + `AuditSource.PRIZE_LOT_RESERVED` + Alembic `0032_audit_source_prize_lot_reserved` — разметка под use-case-ы спинов C.6.c).
- **В работе:** —.
- **Дальше:** C.6.c — хаппи-патх резервирования `update_status(lot_id, RESERVED)` + audit `PRIZE_LOT_RESERVED`; C.6.d — race-fallback (MVP — LengthGain-выпадение при `PrizeLotStatusTransitionError`); C.6.e — integration-тест реального спина с SQLAlchemy-репо. Далее C.7—C.11 (см. `docs/current_tasks.md`). Все оставшиеся блоки разбиты на микрошаги `C.X.a / .b / .c / ...` (пере-декомпозиция 2026-05-11 под уменьшенные лимиты).

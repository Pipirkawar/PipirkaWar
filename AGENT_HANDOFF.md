...
- **C.7.c.fix (этот коммит, hand-off от предыдущего агента):** В предыдущих коммитах `f0344c6` (C.7.c smoke-тест) и `c05bb48` (C.7.b cron) HANDOFF утверждал «make ci зелёный», но на чистом чекауте `mypy --strict` падал двумя `[attr-defined]`-ошибками в `tests/unit/infrastructure/scheduler/test_aps.py:1333,1336` (`cmd.currency` / `cmd.idempotency_key` на `list[object]`). Исправлено: `_FakeGeneratePrizeLotsUseCase.commands: list[object] → list[GeneratePrizeLotsCommand]` + аннотирован `execute(command: GeneratePrizeLotsCommand) -> None` (убран `# type: ignore[no-untyped-def]`). `make ci` зелёный на этом коммите: ruff ok, mypy 946 source files (0 issues), import-linter 4/4 kept, pytest 5668 passed / 2 skipped, coverage 95.60%.
- **Активный шаг чек-листа:** **C.7.d** — триггер `GeneratePrizeLots` после крупного донат-зачисления. Два варианта (открытое решение):
    - (А) **прямой вызов** `GeneratePrizeLots.execute(...)` внутри `RecordDonation.execute(...)` (после `apply_increment`), если `pool_after - pool_before >= TRIGGER_THRESHOLD[currency]`. Проще, но связывает use-case-ы — `RecordDonation` начинает зависеть от `GeneratePrizeLots`, а тот — от `IFeeEstimator`/`IPrizeLotRepository`. Транзакция расширяется на оба use-case-а.
    - (Б) **сигнал через порт** `IPrizeLotGeneratorSignal.notify(currency)` (новый абстрактный порт в `domain/monetization/ports`) — `RecordDonation` зовёт `notify(...)`, infra-реализация (`InMemoryPrizeLotGeneratorSignal` + scheduler-worker, перечитывающий очередь сигналов раз в N секунд) поднимает cron-job. Развязывает use-case-ы, но добавляет инфра-плумбинг.
    - **Рекомендация:** (А) — единая UoW, нет нового порта, легко тестируется. (Б) понадобится, если в 4.1-D/E захотим распараллелить генерацию или вынести её в отдельный воркер.
    - Threshold: `1 USDT` = `1_000_000` USDT_DECIMAL; `0.5 TON` = `500_000_000` TON_NANO; `STARS` — без триггера, ждёт cron-а (slow accumulation).
    - Юнит-тесты: donation >= threshold → тригер сработал (use-case вызвался); donation < threshold → нет; donation идемпотентный re-call → cron-trigger тоже идемпотентный (тот же `idempotency_key:event`).
...
- **В работе:** —.
- **Дальше:** C.7.d — триггер после крупного донат-зачисления (хук в `RecordDonation` или signal-poll, см. открытый вопрос выше). Далее C.8—C.11 (см. `docs/current_tasks.md`).

...
- **Активный шаг чек-листа:** **C.6.c** — happy-path резервирования. После picker-а, если `outcome.kind is CRYPTO_LOT` и `outcome.lot_id is not None`, в той же UoW: `await prize_lots.update_status(lot_id=outcome.lot_id, new_status=PrizeLotStatus.RESERVED)` + audit `PRIZE_LOT_RESERVED` (shape: `target_kind="prize_lot"`, `target_id="<lot_id>:reserved"`, `after={lot_id, currency, amount_native, prev_status="active", reserved_at, player_id, spin_kind}`, `actor_id=player.tg_id`, `reason=f"{spin_kind}_roulette_reserve_lot"`, `idempotency_key=f"{root_key}:reserve:{lot_id}"`). Применять и в free, и в paid. Unit-test happy-path: 1 active lot → picker возвращает crypto_lot → use-case делает `update_status(X, RESERVED)` + audit-запись `PRIZE_LOT_RESERVED`. Никакого race-fallback пока (это C.6.d).
- **Готовы:** C.0 (snapshot pivot + sticky HANDOFF), C.1 (Domain `PrizeLot` aggregate + VO `FeeBufferAmount` + ports + errors + 67 unit-тестов), C.2 (Application use-case `GeneratePrizeLots` + 41 unit-тестов + Alembic 0029 audit-source `prize_lot_generated`), C.3 (Alembic `0030_prize_lots` + `PrizeLotORM` + `SqlAlchemyPrizeLotRepository` + 33 integration-тестов), C.4 (`AuditAction.PRIZE_LOT_REFUNDED` + `AuditSource.PRIZE_LOT_REFUNDED` + Alembic `0031_audit_source_prize_lot_refunded`), C.5 (picker `active_lots: Sequence[PrizeLot]` + `RouletteOutcome.crypto_lot(lot_id)` + invariants `kind == CRYPTO_LOT ⇔ lot_id >= 1` + 25 новых unit-тестов + use-case-ы прокинуты с `active_lots=()` на C.5), C.6.a (`AuditAction.PRIZE_LOT_RESERVED` + `AuditSource.PRIZE_LOT_RESERVED` + Alembic `0032_audit_source_prize_lot_reserved` — разметка под use-case-ы спинов C.6.c), **C.6.b** (DI-параметр `prize_lots: IPrizeLotRepository` в `SpinPaidRoulette` / `SpinFreeRoulette`, вызов `prize_lots.list_active(currency=Currency.STARS)` перед picker-ом, проброс `active_lots` в `pick_*_outcome`. MVP-валюта free-рулетки = STARS, composition root в `bot/main.py` обновлён через `SqlAlchemyPrizeLotRepository`, +1 happy-path unit-test «picker возвращает crypto_lot при непустом пуле»).
- **В работе:** —.
- **Дальше:** C.6.d — race-fallback (MVP — LengthGain-выпадение при `PrizeLotStatusTransitionError` из `update_status`); C.6.e — integration-тест реального спина с SQLAlchemy-репо. Далее C.7—C.11 (см. `docs/current_tasks.md`). Все оставшиеся блоки разбиты на микрошаги `C.X.a / .b / .c / ...` (пере-декомпозиция 2026-05-11 под уменьшенные лимиты).

...
- **Активный шаг чек-листа:** **C.6.d** — race-fallback при резервировании лота. Если `update_status(lot_id, RESERVED)` бросает `PrizeLotStatusTransitionError` (другой игрок забронировал лот первым между `list_active()` и `update_status()`), use-case **не** должен валить весь спин: outcome подменяется на `RouletteOutcome.length_gain(length_cm=fallback_cm)` — `fallback_cm` берётся через `random.randint(*length_buckets[N].range)` от уже выбранного через `_roll_length_bucket`-helper-а (либо middle-of-range MVP-эвристика). MVP-стратегия: **сразу LengthGain-fallback**, без retry-картинки — детерминистично, без бесконечного loop-а. Unit-test: `FakePrizeLotRepository.update_status_raises = PrizeLotStatusTransitionError(...)` + assert outcome.kind == LENGTH + `prize_lots.update_status_calls` содержит ровно одну попытку резерва.
- **Готовы:** C.0 (snapshot pivot + sticky HANDOFF), C.1 (Domain `PrizeLot` aggregate + VO `FeeBufferAmount` + ports + errors + 67 unit-тестов), C.2 (Application use-case `GeneratePrizeLots` + 41 unit-тестов + Alembic 0029 audit-source `prize_lot_generated`), C.3 (Alembic `0030_prize_lots` + `PrizeLotORM` + `SqlAlchemyPrizeLotRepository` + 33 integration-тестов), C.4 (`AuditAction.PRIZE_LOT_REFUNDED` + `AuditSource.PRIZE_LOT_REFUNDED` + Alembic `0031_audit_source_prize_lot_refunded`), C.5 (picker `active_lots: Sequence[PrizeLot]` + `RouletteOutcome.crypto_lot(lot_id)` + invariants `kind == CRYPTO_LOT ⇔ lot_id >= 1` + 25 новых unit-тестов + use-case-ы прокинуты с `active_lots=()` на C.5), C.6.a (`AuditAction.PRIZE_LOT_RESERVED` + `AuditSource.PRIZE_LOT_RESERVED` + Alembic `0032_audit_source_prize_lot_reserved`), C.6.b (DI-параметр `prize_lots: IPrizeLotRepository` в use-case-ы спинов + `list_active(STARS)` перед picker-ом + composition root через `SqlAlchemyPrizeLotRepository`), **C.6.c** (happy-path резервирования: если `outcome.kind is CRYPTO_LOT` → `await prize_lots.update_status(lot_id, RESERVED)` в той же UoW что и спин + audit `PRIZE_LOT_RESERVED` с full shape по C.6.a — `target_kind="prize_lot"`, `target_id="<lot_id>:reserved"`, `after={lot_id, currency, amount_native, prev_status="active", reserved_at, player_id, spin_kind}`, `reason="{spin_kind}_roulette_reserve_lot"`, `idempotency_key="{root_key}:reserve:{lot_id}"` для free и `"{root_key}:spin:{i}:reserve:{lot_id}"` для paid. +2 unit-теста happy-path в free/paid. Без race-fallback — это C.6.d).
- **В работе:** —.
- **Дальше:** C.6.e — integration-тест реального спина с SQLAlchemy-репо (`test_spin_free_roulette_use_case`: seed STARS-лот → spin с CRYPTO_LOT-only конфигом → ожидать `PrizeLotORM.status="reserved"` + audit-запись `PRIZE_LOT_RESERVED`). Далее C.7—C.11 (см. `docs/current_tasks.md`). Все оставшиеся блоки разбиты на микрошаги `C.X.a / .b / .c / ...` (пере-декомпозиция 2026-05-11 под уменьшенные лимиты).

...
- **База**: `main = 1601410`.
- **Сессия**: https://app.devin.ai/sessions/f1baa58dac484f88a557431fc74959bb (приёмка после E.9, работа E.11a+; предыдущая — https://app.devin.ai/sessions/ba3b3f787335465f995742b5aba6799c E.6–E.9)
...
| **E.11a** | Persistence: Alembic `0037` — `payout_freeze` singleton + `prize_lots.winner_id` + покрывающий индекс + `PayoutFreezeORM` + `SqlAlchemyPayoutFreezeRepository` + SQL `sum_claimed_in_window` / `oldest_claimed_at_in_window` + `ClaimPrize` передаёт `winner_id` + integration/unit-тесты | 🔄 in_progress (этот коммит) |
| **E.10** | Hook `EvaluatePayoutLimit` + freeze-check в `ClaimPrize.execute(...)` (over-limit → queue; frozen → reject) | ⏳ pending |
| **E.11b** | Persistence: queue (расширение `prize_lots.status=QUEUED` или отдельная таблица) + Alembic | ⏳ pending |
...
E.11a завершён (этот коммит): схема `payout_freeze` + `prize_lots.winner_id` + `SqlAlchemyPayoutFreezeRepository` + SQL `sum_claimed_in_window`/`oldest_claimed_at_in_window` + `ClaimPrize` передаёт `winner_id`. Следующий шаг — **E.10**: hook в `ClaimPrize.execute(...)` сначала `IPayoutFreezeRepository.get_state()` (frozen → `ClaimPrizePayoutsFrozenError`), потом `EvaluatePayoutLimit.execute(...)` (over-limit → reject или queue). Затем E.11b (очередь, если нужна) → E.12–E.20.

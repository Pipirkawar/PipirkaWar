...
| **E.10** | Hook `EvaluatePayoutLimit` + freeze-check в `ClaimPrize.execute(...)` + new errors `ClaimPrizePayoutsFrozenError` / `ClaimPrizeOverLimitError` + composition root wires `SqlAlchemyPayoutFreezeRepository` + `EvaluatePayoutLimit` | 🔄 in_progress (этот коммит) |

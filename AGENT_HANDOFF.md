# AGENT HANDOFF — Спринт 4.1-D (шаг D.5/D.15)
...
- **D.5 (этот коммит): Infrastructure TON-RPC адаптеры.**
...
- Файл: закончил D.5; следующий — **D.6 «Bot-handler /link_wallet»** (`src/pipirik_wars/bot/...`): `/link_wallet`-команда + TON Connect deeplink + callback на `tonconnect_proof` → `LinkWallet.execute(...)`. Локали RU + EN.
- Где брать ТЗ: `docs/current_tasks.md` чек-лист D.6; ГДД §12.6.4 «Привязка кошелька».
- Composition root (`bot/main.py`) — пока не трогаем: подключение `TonRpcAdapter` / `TonRpcFeeEstimator` / `JettonUsdtProvider` отложено на **D.10** (single composition-root-коммит).

## Состояние ветки
- Ветка: `devin/1778501374-sprint-4-1-D-ton-connect-usdt-claim-prize`
- База: `main` (= `db8e630 Merge pull request #131`)
- Последний коммит: `feat(4.1-D): D.5 — Infrastructure TonRpcAdapter + TonRpcFeeEstimator + JettonUsdtProvider + 56 unit-тестов`
- Незакоммиченные изменения: нет (после коммита)
- CI прогонялся локально: ДА, **make ci зелёный** (5792 passed, 95.63% coverage).

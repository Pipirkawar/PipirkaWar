...
- **D.7.b (этот коммит):** `/claim_prize <lot_id>` handler (`src/pipirik_wars/bot/handlers/claim_prize.py`) — личка-only, pre-loads лот через `IPrizeLotRepository.get_by_id(...)` (ветки `not-found` / `already-claimed` / `not-reserved`), pre-loads кошелёк через `IWalletRepository.get_by_player_and_currency(...)` (ветка `wallet-not-linked`), вызывает `ClaimPrize.execute(...)`, рендерит `success` / `refund`. Race-condition защита: ловит `PrizeLotNotFoundError` / `PrizeLotStatusTransitionError` / `WalletNotLinkedError` от use-case и рендерит соответствующие ветки. 23 handler-теста (`tests/unit/bot/handlers/test_claim_prize.py`).

## На каком файле / задаче остановился
- Файл: закончил D.7.b; следующий — **D.7.c «Кнопка ‘Забрать приз’ в roulette-result + callback handler + тесты»** (`src/pipirik_wars/bot/handlers/claim_prize.py` callback + `SpinFreeRoulettePresenter`/`SpinPaidRoulettePresenter`).
- *Note for D.10.c:* refund-ветка пока рендерит `actual_fee_native=0` (placeholder через `_actual_fee(...)` в handler-е) — `ClaimPrizeResult.refund_*` контракт расширим в D.10.c (или раньше, если потребуется).
...
- Последний коммит: `feat(4.1-D): D.7.b — /claim_prize <lot_id> handler + 23 handler-тестов`.

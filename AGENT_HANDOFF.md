...
- **Активный шаг чек-листа:** **C.6** — резервирование лота в use-case-ах спинов (`SpinPaidRoulette` / `SpinFreeRoulette`-flow): когда picker возвращает `RouletteOutcome.crypto_lot(lot_id=...)`, в той же UoW делать `IPrizeLotRepository.update_status(lot_id, ACTIVE → RESERVED)` + audit `PRIZE_LOT_RESERVED`. Реальный `IPrizeLotRepository.list_active(currency)` прокидывается перед picker-ом. Fallback на race (второй игрок выбрал тот же лот) — всё ещё открытый вопрос (retry picker vs LengthGain).
...
- **В работе:** —.
- **Дальше:** C.6 — резервирование лота + audit `PRIZE_LOT_RESERVED` (см. `docs/current_tasks.md`).

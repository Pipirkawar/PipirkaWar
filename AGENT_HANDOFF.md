...
- **Активный шаг чек-листа:** **C.7.c** — smoke-тест cron-flow `GeneratePrizeLots` через APScheduler-fake (`apscheduler.schedulers.base.BaseScheduler` или `AsyncIOScheduler` с `add_job(next_run_time=immediately)`). Должно гонять без БД: in-memory `IPrizePoolRepository` + `IPrizeLotRepository` + `InMemoryFeeEstimator` + fake clock; trigger срабатывает один раз → `GeneratePrizeLots.execute` вызвался один раз → лоты появились в репозитории. Цель — функциональная проверка, что cron-registration + late-bound factory + callback пишут в repo через UoW корректно. **Важно**: НЕ повторять контракт C.2 (use-case-юнит-тесты) — только проверить, что cron-механика и DI-сборка не сломаны.
...
- **C.7.b** (готов в этом коммите): hourly cron-entry `GeneratePrizeLots` per currency. В `infrastructure/scheduler/aps.py`:
...
- **В работе:** —.
- **Дальше:** C.7.c — smoke-тест cron-flow через APScheduler-fake (in-memory repo, fake clock); C.7.d — триггер после крупного донат-зачисления (хук в `RecordDonation` или signal-poll, см. открытый вопрос). Далее C.8—C.11 (см. `docs/current_tasks.md`).

...
E.6 завершён локально (`make lint typecheck imports` зелёный; полный `pytest -q --no-cov` → 6456 passed, 2 skipped — прирост на 25 тестов от E.5 из предыдущего коммита). Следующий шаг — **E.7**: `FreezePayouts(admin_id, reason)` / `UnfreezePayouts(admin_id)` use-cases (в `application/monetization/`), пишут в `IPayoutFreezeRepository` (E.4) + audit через `IAuditLogger` (`AdminAuditAction.ADMIN_FREEZE_PAYOUTS` / `ADMIN_UNFREEZE_PAYOUTS` из E.3). TOTP-проверка живёт в bot-handler-е (E.14), не в use-case-е. Use-case требует только `is_super_admin`-проверки (через `IPlayerRepository.get_by_id` + проверка `Player.is_super_admin`, аналогично другим admin-use-case-ам).

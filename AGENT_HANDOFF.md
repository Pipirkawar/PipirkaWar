...
- **База**: `main = 1601410`.
- **Сессия**: https://app.devin.ai/sessions/ba3b3f787335465f995742b5aba6799c (приёмка после E.6 + E.7 + E.8 + E.9; предыдущая — https://app.devin.ai/sessions/31de1361f63346afa0cbf75425553331 до E.6 включительно)
...
| **E.9** | Application: `GetPrizePoolStatus(admin_id) -> StatusReport` (read-only + audit `ADMIN_PRIZE_POOL_VIEWED`) | 🔄 in_progress (этот коммит) |
...
E.9 завершён локально (`make lint typecheck imports` зелёный; полный `pytest -q --no-cov` → 6512 passed, 2 skipped). Следующий шаг — **E.10/E.11**: persistence + hook в `ClaimPrize`. По issue с порядком (E.11 ДОЛЖЕН идти ДО E.10, см. блок «Проблема winner_id» ниже): начать с **E.11** — Alembic-миграция: (a) `payout_freeze` singleton-таблица (`id PK fixed=1`, `is_frozen BOOL`, `frozen_by_admin_id BIGINT nullable`, `frozen_at TIMESTAMPTZ nullable`, `reason TEXT nullable`, CHECK `id=1`); (b) `prize_lots.winner_id BIGINT nullable + index (winner_id, currency, status, claimed_at)`; (c) `prize_lots.status='queued'` (или отдельная `payout_queue`-таблица — выбрать по обсуждению). Затем `SqlAlchemyPayoutFreezeRepository` + раскоммитить SQL-реализации `sum_claimed_in_window` / `oldest_claimed_at_in_window` поверх `winner_id`. После этого E.10 — `ClaimPrize.execute(...)` сначала `IPayoutFreezeRepository.get_state()` (frozen → `ClaimPrizePayoutsFrozenError`), потом `EvaluatePayoutLimit.execute(...)` (over-limit → queue или reject). +integration-тесты Alembic + unit-тесты ClaimPrize-hook-ов.

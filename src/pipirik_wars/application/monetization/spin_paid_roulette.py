...
5b. **Donation → призовой пул** (Спринт 4.1-B / Шаг B.5, ГДД §12.6.1):
    `RecordDonation.execute(currency=STARS, payment_amount_native=cost_stars,
    idempotency_key=command.idempotency_key)`. 10% от списанных ⭐ идет в
    `PrizePool.stars` через `IPrizePoolRepository.apply_increment(...)` +
    audit-запись `PRIZE_POOL_INCREMENT` с `target_id=f"{key}:donation"`,
    `idempotency_key=f"{key}:prize_pool"`. При `cost_stars < 10`
    (`floor`-округление) донат = 0, audit не пишется — see
    `RecordDonation`-docstring. Вызов внутри caller-UoW (`async with
    self._uow`) — пул-инкремент / audit / платёж / spin-записи
    атомарны. Любая ошибка из `RecordDonation` пробрасывается наверх
    без catch-ов — инвариант «platform или взяла платёж И внесла 10% в пул,
    или ничего не случилось» (UoW rollback).
6. **Pick исхода `n` раз** (`n=1` для `SINGLE`, `n=pack10_spins`
   ...
   active_lots=())`. На 4.1-A/B `active_lots` всегда пуст
...
class SpinPaidRoulette:
    ...
    async def execute(self, command: SpinPaidRouletteCommand) -> SpinPaidRouletteResult:
        ...
        async with self._uow:
            ...
            # Step 5b — 10% платежа → призовой пул (ГДД §12.6.1, Спринт 4.1-B).
            # Тот же `idempotency_key`, что и у платежа: `RecordDonation` внутри
            # формирует аудит-`idempotency_key` как
            # `f"{key}:prize_pool"` — отдельный scope от `:payment`. UoW уже
            # выше открыта — инкремент пула, audit-запись и платёж атомарны
            # («потерянного доната» нет). Для Stars-платежа `< 10 ⭐` донат = 0
            # (`floor`-округление), `applied=False` — audit-запись не пишется.
            await self._record_donation.execute(
                RecordDonationCommand(
                    currency=Currency.STARS,
                    payment_amount_native=cost_stars,
                    idempotency_key=command.idempotency_key,
                )
            )

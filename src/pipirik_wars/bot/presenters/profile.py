...
С 1.5.E legacy `render_full_nick` удалён: forest-презентер тоже
переехал на `IMessageBundle` и собирает «полный ник» через
`title_message_key(...)`.
"""
...
# Локализованное имя титула берётся из `IMessageBundle` через
# `title_message_key(...)`. Hardcoded маппинги в Python больше не нужны.

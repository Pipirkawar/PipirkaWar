"""Расширение whitelist ``audit_log.source`` источником ``prize_lot_claimed`` (Спринт 4.1-D).

``AuditSource.PRIZE_LOT_CLAIMED`` — выплата зарезервированного лота
игроку (ГДД §12.6.4). Пишется use-case-ом ``ClaimPrize`` при
``RESERVED → CLAIMED``.
"""
...
revision: str = "0033_audit_source_prize_lot_claimed"
down_revision: str | Sequence[str] | None = "0032_audit_source_prize_lot_reserved"
